...
import { CLIPBOARD_CONTENT_QUOTE_PREFIX, SETTING_CONFIG_DEFAULT, INITIAL_FILE_SIZE, PROMPT_CONTENT_SEPARATOR, PROMPT_TRIGGER_PREFIX, RESOURCE_TRIGGER_PREFIX } from '$lib/constants';
import { ContentPartType, FileExtensionText, KeyboardKey, MimeTypeText, SpecialFileType } from '$lib/enums';
...
import { AudioRecorder, convertToWav, createAudioFile, isAudioRecordingSupported } from '$lib/utils/browser-only';
import { onMount } from 'svelte';
let { attachments = [], class: className = '', disabled = false, isLoading = false, placeholder = t('Type a message...'), showMcpPromptButton = false, showAddButton = true, showModelSelector = true, uploadedFiles = $bindable([]), value = $bindable(''), onAttachmentRemove, onFilesAdd, onStop, onSubmit, onSystemPromptClick, onUploadedFileRemove, onUploadedFilesChange, onValueChange } = $props();
// Component References
let audioRecorder;
let chatFormActionsRef = $state(undefined);
let fileInputRef = $state(undefined);
let pickersRef = $state(undefined);
...
let preSelectedResourceUri = $state(undefined);
...
let conversationModel = $derived(chatStore.getConversationModel(activeMessages()));
...
let hasAttachments = $derived((attachments && attachments.length > 0) || (uploadedFiles && uploadedFiles.length > 0));
...
export function checkModelSelected() {
    if (!hasModelSelected) {
        chatFormActionsRef?.openModelSelector();
        return false;
    }
...
function handleInput() {
    ...
    else if (value.startsWith(RESOURCE_TRIGGER_PREFIX) &&
        hasServers &&
...
function handleKeydown(event) {
    ...
    if (event.key === KeyboardKey.ENTER && !event.shiftKey && !isIMEComposing(event)) {
        ...
        if (sendOnEnter || isModifier) {
            ...
            if (!canSubmit || disabled || hasLoadingAttachments)
                return;
            onSubmit?.();
        }
    }
    ...
    if (text.startsWith(CLIPBOARD_CONTENT_QUOTE_PREFIX)) {
        ...
        if (parsed.textAttachments.length > 0 || parsed.mcpPromptAttachments.length > 0) {
            ...
            if (parsed.textAttachments.length > 0) {
                const attachmentFiles = parsed.textAttachments.map((att) => new File([att.content], att.name, {
...
function handlePromptLoadError(placeholderId, error) {
    uploadedFiles = uploadedFiles.map((f) => f.id === placeholderId ? { ...f, isLoading: false, loadError: error } : f);
    onUploadedFilesChange?.(uploadedFiles);
}

...
import { modelsStore, modelOptions, selectedModelId, selectedModelName } from '$lib/stores/models.svelte';
import { isRouterMode, serverError } from '$lib/stores/server.svelte';
import { activeMessages } from '$lib/stores/conversations.svelte';
let { disabled = false, forceForegroundText = false, hasAudioModality = $bindable(false), hasVideoModality = $bindable(false), hasVisionModality = $bindable(false), hasModelSelected = $bindable(false), isSelectedModelInCache = $bindable(true), submitTooltip = $bindable(''), useGlobalSelection = false } = $props();
let isRouter = $derived(isRouterMode());
let isOffline = $derived(!!serverError());
let conversationModel = $derived(chatStore.getConversationModel(activeMessages()));
let lastSyncedConversationModel = null;
...
$effect(() => {
    if (conversationModel && conversationModel !== lastSyncedConversationModel) {
        ...
        if (first)
            modelsStore.selectModelById(first.id);
    }
});

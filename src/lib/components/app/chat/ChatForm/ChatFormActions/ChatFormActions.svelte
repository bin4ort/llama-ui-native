<script>import { FileTypeCategory, MessageRole } from '$lib/enums';
import { mcpStore } from '$lib/stores/mcp.svelte';
import { config } from '$lib/stores/settings.svelte';
import { activeMessages, conversationsStore } from '$lib/stores/conversations.svelte';
import { activeProcessingState, isChatStreaming, isLoading as chatIsLoading } from '$lib/stores/chat.svelte';
import { getFileTypeCategory } from '$lib/utils';
import { page } from '$app/state';
let { canSend = false, canSubmit = false, class: className = '', disabled = false, isLoading = false, isReasoning = false, isRecording = false, showAddButton = true, showModelSelector = true, uploadedFiles = [], onFileUpload, onMicClick, onStop, onSystemPromptClick, onMcpPromptClick, onMcpResourcesClick } = $props();
...
let hasAudioAttachments = $derived(uploadedFiles.some((file) => getFileTypeCategory(file.type) === FileTypeCategory.AUDIO));
let shouldShowRecordButton = $derived(hasAudioModality && !canSubmit && !hasAudioAttachments && currentConfig.autoMicOnEmpty);
let selectorModelRef = $state(undefined);
...
let activeMessage = $derived(conversationsStore.activeMessages[conversationsStore.activeMessages.length - 1]);
let hasProcessedTokens = $derived.by(() => {
    ...
    if (totalHistoricalTokens > 0)
        return true;
    if (!chatIsLoading() && !isChatStreaming())
        return false;
    const processingState = activeProcessingState();
    if (!processingState)
        return false;
    const livePromptTokens = Math.max(processingState.promptTokens ?? 0, processingState.promptProgress?.processed ?? 0);

...
let { class: className = '', isOpen = false, searchQuery = '', onClose, onResourceSelect, onBrowse } = $props();
let resources = $state([]);
...
function isResourceAttached(uri) {
...
let filteredResources = $derived.by(() => {
    ...
    return sortedResources.filter((resource) => resource.name.toLowerCase().includes(query) ||

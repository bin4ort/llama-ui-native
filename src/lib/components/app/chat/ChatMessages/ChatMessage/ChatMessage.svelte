...
let { class: className = '', message, toolMessages = [], isLastAssistantMessage = false, isLastUserMessage = false, nextAssistantMessage = null, siblingInfo = null } = $props();
const chatActions = getChatActionsContext();
let deletionInfo = $state(null);
...
let editedExtras = $derived(message.extra ? [...message.extra] : []);
...
setMessageEditContext({
    ...
    setShouldBranchAfterEdit: (value) => {
...
let mcpPromptExtra = $derived.by(() => {
    ...
    if (message.content.trim())
        return null;
    if (!message.extra || message.extra.length !== 1)
        return null;
...
async function getMergedExtras() {
...
function handleShowDeleteDialogChange(show) {

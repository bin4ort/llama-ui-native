...
import { DEFAULT_LANGUAGE, FILE_PATH_SEPARATOR_REGEX, TEXT_LANGUAGE_PREFIX_REGEX } from '$lib/constants';
import { getFileTypeByExtension, tryParseToolResultObject } from '$lib/utils';
import { parseToolArgs } from './_shared';
export function parseWriteFileMeta(section) {
    ...
    const language = getFileTypeByExtension(rawPath)?.replace(TEXT_LANGUAGE_PREFIX_REGEX, '') ?? DEFAULT_LANGUAGE;
    const resultObj = tryParseToolResultObject(section.toolResult);
    const bytesWritten = resultObj && Number.isFinite(Number(resultObj.bytes)) ? Number(resultObj.bytes) : undefined;

<script>import { createAutoScrollController } from '$lib/hooks/use-auto-scroll.svelte';
...
import { chatStore, errorDialog, isLoading, isChatStreaming } from '$lib/stores/chat.svelte';
import { conversationsStore, activeMessages, activeConversation } from '$lib/stores/conversations.svelte';
import { config } from '$lib/stores/settings.svelte';
import { serverLoading, serverError } from '$lib/stores/server.svelte';
import { parseFilesToMessageExtras } from '$lib/utils/browser-only';
import { onDestroy, onMount, tick } from 'svelte';
import { LANDING_SETTLE_MAX_MS, LANDING_STABLE_FRAMES } from '$lib/constants';
...
let emptyFileNames = $state([]);
let initialMessage = $state('');
let showDeleteDialog = $state(false);
let showEmptyFileDialog = $state(false);
let isEmpty = $derived(showCenteredEmpty && !activeConversation() && activeMessages().length === 0 && !isLoading());
...
async function handleSendMessage(message, files) {
    ...
    if (result?.emptyFiles && result.emptyFiles.length > 0) {
        ...
        if (files) {
            ...
            fileUpload.uploadedFiles = fileUpload.uploadedFiles.filter((file) => !emptyFileNamesSet.has(file.name));
        }
        return false;
    }
...
let lastScrolledConversationId = null;
...
async function handleMessagesReady(messageCount) {
    if (messageCount === 0)
        return;
    const id = activeConversation()?.id ?? null;
    if (!id || id === lastScrolledConversationId)
        return;
    ...
    const settle = () => {
        ...
        lastHeight = height;
        if (stableFrames >= LANDING_STABLE_FRAMES)
            return;
        if (performance.now() - started > LANDING_SETTLE_MAX_MS)
            return;
...
function handleSendLikeScroll() {
    ...
    setTimeout(() => {
        ...
        if (!container)
            return;
        const lastUserBubble = container.querySelector('.chat-message:nth-last-child(2) .chat-message-user .chat-message-user-bubble');
        if (isMobile.current) {
...
function handleErrorDialogOpenChange(open) {
    if (!open) {
        chatStore.dismissErrorDialog();
    }
}
async function handleSystemPromptAdd(draft) {
...
$effect(() => {
    const shouldDisableAutoScroll = config().disableAutoScroll || (isMobile.current && isCurrentConversationLoading);

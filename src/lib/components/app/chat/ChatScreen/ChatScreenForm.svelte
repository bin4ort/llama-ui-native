...
let { class: className, disabled = false, initialMessage = '', isLoading = false, onFileRemove, onFileUpload, onSend, onStop, onSystemPromptAdd, uploadedFiles = $bindable([]) } = $props();
let chatFormRef = $state(undefined);
let formWrapperEl = $state();
...
async function handleSubmit() {
    if ((!message.trim() && uploadedFiles.length === 0) || disabled || hasLoadingAttachments)
        return;
    if (!chatFormRef?.checkModelSelected())
        return;

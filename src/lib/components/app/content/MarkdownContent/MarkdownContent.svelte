...
import { getHastNodeId, getMdastNodeHash, isAppendMode, getCodeInfoFromTarget } from './markdown-utils';
import { preprocessLaTeX, getImageErrorFallbackHtml, copyCodeToClipboard, copyToClipboard } from '$lib/utils';
import { IMAGE_NOT_ERROR_BOUND_SELECTOR, DATA_ERROR_BOUND_ATTR, DATA_ERROR_HANDLED_ATTR, BOOL_TRUE_STRING, CODE_BLOCK_HEADER_CLASS, MERMAID_WRAPPER_CLASS, MERMAID_BLOCK_CLASS, MERMAID_SYNTAX_ATTR, MERMAID_RENDERED_ATTR, SVG_WRAPPER_CLASS, SVG_BLOCK_CLASS, SVG_LANGUAGE, XML_LANGUAGE, SVG_TAG_PREFIX, SVG_SOURCE_ATTR, SVG_RENDERED_ATTR, SVG_INLINE_SHADOW_STYLE, TOGGLE_SOURCE_BTN_CLASS, DIAGRAM_VIEW_MODE_ATTR, DIAGRAM_VIEW_RENDERED, DIAGRAM_VIEW_SOURCE } from '$lib/constants';
import { ColorMode, UrlProtocol } from '$lib/enums';
import { FileTypeText } from '$lib/enums/files.enums';
import { highlightCode, detectIncompleteCodeBlock } from '$lib/utils';
...
let { content, attachments, class: className = '', disableMath = false } = $props();
let containerRef = $state();
let renderedBlocks = $state([]);
let unstableBlockHtml = $state('');
let incompleteCodeBlock = $state(null);
const streamingSvgCode = $derived.by(() => {
    const block = incompleteCodeBlock;
    if (!block)
        return null;
    if (block.language === SVG_LANGUAGE)
        return block.code;
...
const streamingCodeHtml = $derived(incompleteCodeBlock
...
let streamingSvgHost = $state(null);
// While the zoom dialog is open on a streaming svg, mirror the live render into it
$effect(() => {
    if (svgPreviewLive && liveSvgHtml)
        mermaidPreviewSvgHtml = liveSvgHtml;
});
// Mount the streaming svg into its shadow host on every chunk so it renders live
$effect(() => {
    if (streamingSvgHost)
        mountSvgShadow(streamingSvgHost, liveSvgHtml, SVG_INLINE_SHADOW_STYLE);
});
let streamingCodeScrollContainer = $state();
...
let processor = $derived(() => {
    ...
    let proc = remark().use(remarkGfm); // GitHub Flavored Markdown
    if (!disableMath) {
...
function cleanupEventListeners() {
    ...
    const previewButtons = containerRef.querySelectorAll('.preview-code-btn');
 ...
 */
async function transformMdastNode(processorInstance, node, index) {
    ...
    const transformedRoot = (await processorInstance.run(singleNodeRoot));
 ...
 */
function handlePreviewDialogOpenChange(open) {
...
async function processMarkdown(markdown) {
    ...
    if (incompleteBlock) {
        ...
        if (prefixMarkdown.trim()) {
            ...
            const ast = processorInstance.parse(normalizedPrefix);
            const mdastChildren = ast.children ?? [];
            const nextBlocks = [];
            ...
            for (let index = 0; index < mdastChildren.length; index++) {
                ...
                const id = getHastNodeId({ position: child.position }, index);
 ...
 */
async function handleMermaidClick(event) {
    ...
    if (svgCopyBtn || svgPreviewBtn) {
        ...
        if (!wrapper)
            return;
        const preElement = wrapper.querySelector(`pre.${SVG_BLOCK_CLASS}[${SVG_SOURCE_ATTR}]`);
        if (!preElement)
            return;
        if (svgCopyBtn) {
            event.preventDefault();
    ...
    if (target.closest(`.${CODE_BLOCK_HEADER_CLASS}`))
        return;
 ...
 */
function handleMermaidPreviewOpenChange(open) {
...
async function renderMermaidDiagrams() {
    if (!containerRef)
        return;
    const nodes = containerRef.querySelectorAll(`pre.${MERMAID_BLOCK_CLASS}:not([${MERMAID_RENDERED_ATTR}])`);
    if (nodes.length === 0)
        return;
...
function renderSvgDiagrams() {
    if (!containerRef)
        return;
    const nodes = containerRef.querySelectorAll(`pre.${SVG_BLOCK_CLASS}:not([${SVG_RENDERED_ATTR}])`);
    if (nodes.length === 0)
        return;
    nodes.forEach((node) => {
...
function handleImageError(event) {
    ...
    if (img.src.startsWith(UrlProtocol.DATA) ||

...
import { MERMAID_WRAPPER_CLASS, MERMAID_SCROLL_CONTAINER_CLASS, MERMAID_BLOCK_CLASS, MERMAID_LANGUAGE, MERMAID_SYNTAX_ATTR, MERMAID_ID_ATTR, DIAGRAM_VIEW_MODE_ATTR, DIAGRAM_VIEW_RENDERED } from '$lib/constants';
import { createBlockHeader, createCopyButton, createPreviewButton, createToggleSourceButton, createSourceView, createWrapper, generateBlockId } from './code-block-utils';
/**
 * Rehype plugin to enhance mermaid diagram blocks with wrapper, header, and action buttons.
 ...
 * - Preview button
 */
export const rehypeEnhanceMermaidBlocks = () => {
    return (tree) => {
        visit(tree, 'element', (node, index, parent) => {
            ...
            if (!Array.isArray(className))
                return;
            const isMermaid = className.some((cls) => typeof cls === 'string' && cls === MERMAID_BLOCK_CLASS);
            if (!isMermaid)
                return;
            ...
            const preservedCode = node.data?.sourceCode;
            const sourceView = createSourceView(preservedCode, diagramText, MERMAID_LANGUAGE);
            const wrapper = createWrapper(header, node, MERMAID_WRAPPER_CLASS, MERMAID_SCROLL_CONTAINER_CLASS, {
                [MERMAID_ID_ATTR]: mermaidId,

 ...
 */
export function createPreTransform(languages, targetClass, contentGuard) {
    const codeClasses = (Array.isArray(languages) ? languages : [languages]).map((language) => `language-${language}`);
    return () => {
        return (tree) => {
            visit(tree, 'element', (node, index, parent) => {
                ...
                if (!Array.isArray(className))
                    return;
                const matches = className.some((cls) => typeof cls === 'string' && codeClasses.includes(cls));
                if (!matches)
                    return;

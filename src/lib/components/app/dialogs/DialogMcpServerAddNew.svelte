...
import { BEARER_PREFIX, BOOL_FALSE_STRING, BOOL_TRUE_STRING, DISMISSED_RECOMMENDED_MCP_SERVERS_LOCALSTORAGE_KEY, MCP_SERVER_ID_PREFIX, RECOMMENDED_MCP_SERVERS, REDACTED_HEADERS } from '$lib/constants';
...
let selectedRecommendationId = $derived.by(() => {
    ...
    return (RECOMMENDED_MCP_SERVERS.find((rec) => canonicalizeServerUrl(rec.url) === targetCanonical)
        ?.id ?? null);
});
let selectedRecommendation = $derived(selectedRecommendationId
...
let bearerTokenFilled = $derived.by(() => {
    ...
    const bearer = pairs.find((p) => REDACTED_HEADERS.has(p.key.trim().toLowerCase()) &&
...
let newServerHeaderPairsValid = $derived(parseHeadersToArray(newServerHeaders).every((p) => p.key.trim() && p.value.trim()));
let canSave = $derived(!newServerUrlError && newServerHeaderPairsValid && (!authRequired || bearerTokenFilled));
// Backward-compatible read: older versions stored a JSON array of dismissed ids.
function readRecommendationsDismissed() {
    if (!browser)
        return false;
    const raw = localStorage.getItem(DISMISSED_RECOMMENDED_MCP_SERVERS_LOCALSTORAGE_KEY);
    if (!raw)
        return false;
    if (raw === BOOL_TRUE_STRING)
        return true;
    if (raw === BOOL_FALSE_STRING)
        return false;
    try {
...
function writeRecommendationsDismissed(dismissed) {
    recommendationsDismissed = dismissed;
    if (browser) {
        localStorage.setItem(DISMISSED_RECOMMENDED_MCP_SERVERS_LOCALSTORAGE_KEY, dismissed ? BOOL_TRUE_STRING : BOOL_FALSE_STRING);
    }
}
let recommendationsDismissed = $state(readRecommendationsDismissed());
...
let unconfiguredRecommendations = $derived.by(() => {
    const configuredCanonicals = new Set(mcpStore.getServers().map((s) => canonicalizeServerUrl(s.url)));
    return RECOMMENDED_MCP_SERVERS.filter((rec) => !configuredCanonicals.has(canonicalizeServerUrl(rec.url)));
});
let recommendationsToShow = $derived(recommendationsDismissed ? [] : unconfiguredRecommendations);
function handleRecommendationClick(recommendedId) {
...
function saveNewServer() {
    ...
    mcpStore.addServer({
        ...
        displayName: newServerName.trim() && newServerName.trim() !== nameAutoFilled.trim()
            ? newServerName.trim()
            : undefined,
	...
	<Dialog.Content class="sm:max-w-2xl">
		...
		<form onsubmit={handleSubmit} class="contents">
			...
			<Dialog.Footer>
				...
				<Button variant="default" size="sm" type="submit" disabled={!canSave} aria-label={t("Add")}>
					{t("Add")}
				</Button>
			</Dialog.Footer>
		</form>

	...
	<Collapsible.Root bind:open={isExpanded} class={className}>
		<div class="space-y-2">
			...
			>
				...
				<span>{t("Connection Log")} ({logs.length})</span>

				{#if connectionTimeMs !== undefined}
					<span class="ml-1">{t("Connected in")} {connectionTimeMs}ms</span>
				{/if}
			</Collapsible.Trigger>
		</div>
			...
			>
				{#each logs as log (log.timestamp.getTime() + log.message)}
					...
					{#if log.details !== undefined}
						<details class="ml-11">
							<summary class="cursor-pointer text-[10px] text-muted-foreground"> {t("details")} </summary>

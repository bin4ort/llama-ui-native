...
let { template, onResolve, onCancel } = $props();
...
const fetchCompletions = debounce(async (argName, value) => {
    ...
    try {
        const result = await mcpStore.getResourceCompletions(template.serverName, template.uriTemplate, argName, value);
...
<form onsubmit={handleSubmit} class="space-y-3">
	...
	<div class="flex justify-end gap-2 pt-1">
		...
		<Button size="sm" type="submit" disabled={!isComplete}>{t("Read Resource")}</Button>
	</div>
</form>

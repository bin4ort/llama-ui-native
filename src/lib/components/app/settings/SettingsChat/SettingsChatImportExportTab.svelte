<script>import { createMessageCountMap } from '$lib/utils';
import { settingsStore } from '$lib/stores/settings.svelte';
import { conversationsStore, conversations } from '$lib/stores/conversations.svelte';
import { toast } from 'svelte-sonner';
import { HtmlInputType, FileExtensionText } from '$lib/enums';
let exportedConversations = $state([]);
let importedConversations = $state([]);
let showExportSummary = $state(false);
let showImportSummary = $state(false);
let showExportDialog = $state(false);
let showImportDialog = $state(false);
let availableConversations = $state([]);
let messageCountMap = $state(new Map());
let fullImportData = $state([]);
...
async function handleExportClick() {
    try {
        ...
        const conversationsWithMessages = await Promise.all(allConversations.map(async (conv) => {
...
async function handleExportConfirm(selectedConversations) {
    try {
        const allData = await Promise.all(selectedConversations.map(async (conv) => {
            const messages = await conversationsStore.getConversationMessages(conv.id);
...
async function handleImportConfirm(selectedConversations) {
    try {

<script lang="ts">
	...
	import { APP_VERSION, APP_BUILD } from '$lib/constants/app';
	...
	const footerText = `Llama UI Native v${APP_VERSION} (build ${APP_BUILD})`;
</script>
...
<div class="sticky top-2 hidden w-64 flex-col self-start bg-background py-4 md:flex gap-6">
	...
	<div class="mt-auto pt-4 text-center opacity-50 text-[11px]">
		<a href="https://github.com" target="_blank" rel="noopener" class="text-muted-foreground no-underline hover:underline" title={footerText}>{footerText}</a>
	</div>
</div>

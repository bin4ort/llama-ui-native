...
export function useMarqueeSelection(options) {
     ...
     */
    function datasetKey(key = resolveAttributeName()) {
        return key.replace(/-([a-z])/g, (_, c) => c.toUpperCase());
    }
    function decideDragMode(startingRowId, currentlySelected) {
    ...
    function updateMarqueeRect(currentX, currentY) {
        const attr = resolveAttributeName();
        const selector = `[data-${attr}]`;
        const key = datasetKey(attr);
        const selected = options.selectedIds();
    ...
    function rowMouseDown(id, event) {
        if (!options.enabled())
            return;
        if (event.button !== 0)
            return;

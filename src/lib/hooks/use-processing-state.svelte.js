...
export function useProcessingState() {
    ...
    function getETASecs(done, total, elapsedMs) {
        const elapsedSecs = elapsedMs / 1000;
        const progressETASecs = done === 0 || elapsedSecs < 0.5
    ...
    function getProcessingDetails() {
        ...
        if (typeof stateToUse.contextTotal === 'number' &&
            ...
            details.push(`Context: ${stateToUse.contextUsed}/${stateToUse.contextTotal} (${contextPercent}%)`);
        }
        if (stateToUse.outputTokensUsed > 0) {
            ...
            if (stateToUse.outputTokensMax <= 0) {
                ...
                const outputPercent = Math.round((stateToUse.outputTokensUsed / stateToUse.outputTokensMax) * 100);
                details.push(`Output: ${stateToUse.outputTokensUsed}/${stateToUse.outputTokensMax} (${outputPercent}%)`);
            }
        }
     ...
     */
    function getPromptProgressText() {
        if (!processingState?.promptProgress)
            return null;
     ...
     */
    function getLiveProcessingStats() {
        if (processingState?.promptProgress) {
            const { processed, total, time_ms, cache } = processingState.promptProgress;
            const actualProcessed = processed - cache;
            const actualTotal = total - cache;
     ...
     */
    function getLiveGenerationStats() {
        if (!processingState)
            return null;
        ...
        const timeMs = tokensPerSecond && tokensPerSecond > 0 ? (tokensDecoded / tokensPerSecond) * 1000 : 0;
        return {

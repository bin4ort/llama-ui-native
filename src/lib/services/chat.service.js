...
function streamStorageKey(conversationId) {
...
export class ChatService {
     ...
     * @static
     */
    static async generateTitle(message, model, signal) {
        let titleResponse = '';
        try {
     ...
     */
    static async sendMessage(messages, options = {}, conversationId, signal) {
        const { stream, onChunk, onComplete, onError, onConnectionState, onReasoningChunk, onToolCallChunk, onModel, onCompletionId, onTimings, 
        ...
        // Penalty parameters
        repeat_last_n, repeat_penalty, presence_penalty, frequency_penalty, dry_multiplier, dry_base, dry_allowed_length, dry_penalty_last_n, 
        // Other parameters
        samplers, backend_sampling, custom, timings_per_token, 
        // Config options
        disableReasoningParsing, excludeReasoningFromContext, enableThinking, reasoningEffort, continueFinalMessage } = options;
                ...
                if (Array.isArray(msg.content)) {
                    msg.content = msg.content.filter((part) => {
                        if (part.type === ContentPartType.IMAGE_URL) {
                            console.info(`[ChatService] Skipping image attachment in message history (model "${options.model}" does not support vision)`);
                            return false;
                        }
                        return true;
                    });
        ...
        const reasoningBudgetTokens = enableThinking && reasoningEffort ? (REASONING_EFFORT_TOKENS[reasoningEffort] ?? -1) : -1;
        ...
        if (xtc_threshold !== undefined)
            requestBody.xtc_threshold = xtc_threshold;
        if (typ_p !== undefined)
            requestBody.typ_p = typ_p;
        if (repeat_last_n !== undefined)
        ...
        if (dry_base !== undefined)
            requestBody.dry_base = dry_base;
        ...
        if (samplers !== undefined) {
            requestBody.samplers =
                typeof samplers === 'string'
                    ? samplers.split(';').filter((s) => s.trim())
                    : samplers;
        }
        if (backend_sampling !== undefined)
            requestBody.backend_sampling = backend_sampling;
        ...
        try {
            ...
            if (stream) {
                await ChatService.handleStreamResponse(response, onChunk, onComplete, onError, onReasoningChunk, onToolCallChunk, onModel, onCompletionId, onTimings, conversationId, signal, onConnectionState, options.model);
                return;
            }
            else {
                return ChatService.handleNonStreamResponse(response, onComplete, onError, onToolCallChunk, onModel);
            }
        }
        catch (error) {
            ...
            if (error instanceof Error) {
                if (error.name === 'TypeError' && error.message.includes('fetch')) {
                    userFriendlyError = new Error('Unable to connect to server - please check if the server is running');
     ...
     */
    static async areAllSlotsIdle(model, signal) {
        try {
     ...
     */
    static async stopReasoning(completionId, model) {
        if (!completionId) {
            console.error('stopReasoning: no completion id for the active message, cannot target the running completion');
            return false;
        }
        const body = {
            id: completionId,
     ...
     */
    static async cancelServerStream(conversationId, model) {
        if (!conversationId)
            return;
        try {
     ...
     */
    static selectActiveStream(sessions) {
    ...
    static saveStreamState(conversationId, bytesReceived, model) {
        if (!conversationId)
            return;
        try {
            const state = {
    ...
    static getStreamState(conversationId) {
        if (!conversationId)
            return null;
        try {
            ...
            if (!parsed || typeof parsed.bytesReceived !== 'number')
                return null;
            return parsed;
        }
        catch {
    ...
    static clearStreamState(conversationId) {
        if (!conversationId)
            return;
        try {
     ...
     */
    static resumeStreamIdentity(conversationId, state, fallbackModel) {
     ...
     */
    static async resumeStream(conversationId, signal, model) {
        if (!conversationId)
            return null;
    ...
    static async preEncode(messages, model, excludeReasoning, signal) {
        const normalizedMessages = (await Promise.all(messages.map((msg) => {
            if ('id' in msg && 'convId' in msg && 'timestamp' in msg) {
     ...
     */
    static async handleStreamResponse(response, onChunk, onComplete, onError, onReasoningChunk, onToolCallChunk, onModel, onCompletionId, onTimings, conversationId, abortSignal, onConnectionState, streamModel) {
        ...
        const processToolCallDelta = (toolCalls) => {
            if (!toolCalls || toolCalls.length === 0) {
                return;
            }
            aggregatedToolCalls = ChatService.mergeToolCallDeltas(aggregatedToolCalls, toolCalls, toolCallIndexOffset);
        ...
        try {
            ...
            while (true) {
                ...
                // live dropdown
                const resumeResp = await ChatService.resumeStream(conversationId, abortSignal, streamModel).catch(() => null);
            ...
            if (streamFinished) {
                ...
                const finalToolCalls = aggregatedToolCalls.length > 0 ? JSON.stringify(aggregatedToolCalls) : undefined;
     ...
     */
    static async handleNonStreamResponse(response, onComplete, onError, onToolCallChunk, onModel) {
        try {
     ...
     */
    static mergeToolCallDeltas(existing, deltas, indexOffset = 0) {
     ...
     * @static
     */
    static async convertDbMessageToApiChatMessageData(message) {
        ...
        const contentParts = [];
        const textFiles = message.extra.filter((extra) => extra.type === AttachmentType.TEXT);
        ...
        const legacyContextFiles = message.extra.filter((extra) => extra.type === AttachmentType.LEGACY_CONTEXT);
        ...
        const imageFiles = message.extra.filter((extra) => extra.type === AttachmentType.IMAGE);
        ...
        const audioFiles = message.extra.filter((extra) => extra.type === AttachmentType.AUDIO);
        ...
        const videoFiles = message.extra.filter((extra) => extra.type === AttachmentType.VIDEO);
        ...
        const pdfFiles = message.extra.filter((extra) => extra.type === AttachmentType.PDF);
        ...
        const mcpPrompts = message.extra.filter((extra) => extra.type === AttachmentType.MCP_PROMPT);
        for (const mcpPrompt of mcpPrompts) {
            contentParts.push({
                type: ContentPartType.TEXT,
                text: formatAttachmentText(ATTACHMENT_LABEL_MCP_PROMPT, mcpPrompt.name, mcpPrompt.content, mcpPrompt.serverName)
            });
        }
        const mcpResources = message.extra.filter((extra) => extra.type === AttachmentType.MCP_RESOURCE);
        for (const mcpResource of mcpResources) {
            contentParts.push({
                type: ContentPartType.TEXT,
                text: formatAttachmentText(ATTACHMENT_LABEL_MCP_RESOURCE, mcpResource.name, mcpResource.content, mcpResource.serverName)
            });
        }
        const result = {
            role: message.role,
     ...
     */
    static stripReasoningContent(content) {
        const stripFromString = (text) => text.replace(LEGACY_AGENTIC_REGEX.REASONING_BLOCK, '').trim();
     ...
     * @private
     */
    static notifyTimings(timings, promptProgress, onTimingsCallback) {
        if (!onTimingsCallback || (!timings && !promptProgress))
            return;

...
class LlamaUiDatabase extends Dexie {
    [IDXDB_TABLES.conversations];
    [IDXDB_TABLES.messages];
    constructor() {
...
export class DatabaseService {
     ...
     */
    static async createMessageBranch(message, parentId) {
        return await db.transaction('rw', [db[IDXDB_TABLES.conversations], db[IDXDB_TABLES.messages]], async () => {
     ...
     */
    static async createRootMessage(convId) {
        const rootMessage = {
            id: uuid(),
            convId,
            type: 'root',
     ...
     */
    static async createSystemMessage(convId, systemPrompt, parentId) {
     ...
     */
    static async deleteConversation(id, options) {
        await db.transaction('rw', [db[IDXDB_TABLES.conversations], db[IDXDB_TABLES.messages]], async () => {
     ...
     */
    static async reparentDirectChildren(parentId, excludeIds = new Set(), prefetched) {
        ...
        while (newParent && excludeIds.has(newParent)) {
            ...
            const next = prefetched?.get(newParent) ?? (await db[IDXDB_TABLES.conversations].get(newParent));
            if (!next) {
                newParent = undefined;
                break;
            }
     ...
     */
    static async bulkDeleteConversations(ids) {
        ...
        await db.transaction('rw', [db[IDXDB_TABLES.conversations], db[IDXDB_TABLES.messages]], async () => {
            ...
            const requested = new Set(frontier);
     ...
     */
    static async deleteMessageCascading(conversationId, messageId) {
     ...
     */
    static async getConversationMessages(convId) {
     ...
     */
    static async getConversationsWithMessages(convIds) {
        const result = new Map();
        const cleanIds = convIds.filter((id) => typeof id === 'string' && id.length > 0);
        if (cleanIds.length === 0)
            return result;
        ...
        for (const msg of allMessages) {
            ...
            if (bucket)
                ...
                messagesByConv.set(msg.convId, [msg]);
        }
     ...
     */
    static async toggleConversationPin(id) {
     ...
     */
    static async bulkToggleConversationPins(ids) {
        const cleanIds = ids.filter((id) => typeof id === 'string' && id.length > 0);
        const result = new Map();
        if (cleanIds.length === 0)
            return result;
     ...
     */
    static async updateCurrentNode(convId, nodeId) {
     ...
     */
    static async importConversations(data) {
        let importedCount = 0;
        let skippedCount = 0;
        return await db.transaction('rw', [db[IDXDB_TABLES.conversations], db[IDXDB_TABLES.messages]], async () => {
     ...
     */
    static async forkConversation(sourceConvId, atMessageId, options) {
        return await db.transaction('rw', [db[IDXDB_TABLES.conversations], db[IDXDB_TABLES.messages]], async () => {
            ...
            const allMessages = await db[IDXDB_TABLES.messages]
                ...
                .equals(sourceConvId)
                .toArray();
            const pathMessages = filterByLeafNodeId(allMessages, atMessageId, true);
            ...
            const newConv = {
                ...
                mcpServerOverrides: sourceConv.mcpServerOverrides
                    ? sourceConv.mcpServerOverrides.map((o) => ({
                        serverId: o.serverId,
                        enabled: o.enabled
                    }))
                    : undefined

...
import { DEFAULT_MCP_CONFIG, DEFAULT_CLIENT_VERSION, DEFAULT_IMAGE_MIME_TYPE, CORS_PROXY_HEADER_PREFIX, MCP_PARTIAL_REDACT_HEADERS, CORS_PROXY_ENDPOINT } from '$lib/constants';
import { MCPConnectionPhase, MCPLogLevel, MCPTransportType, MCPContentType } from '$lib/enums';
import { buildProxiedUrl, buildProxiedHeaders, getAuthHeaders, sanitizeHeaders, throwIfAborted, isAbortError, createBase64DataUrl, getRequestUrl, getRequestMethod, getRequestBody, summarizeRequestBody, formatDiagnosticErrorMessage, extractJsonRpcMethods } from '$lib/utils';
export class MCPService {
    ...
    static createDiagnosticRequestDetails(input, init, baseInit, requestHeaders, extraRedactedHeaders) {
    ...
    static addRequestHeaders(requestHeaders, headers, useProxy) {
        for (const [key, value] of new Headers(headers).entries()) {
            const proxiedKey = useProxy && !key.toLowerCase().startsWith(CORS_PROXY_HEADER_PREFIX)
    ...
    static getConnectionHints(targetUrl, config, error) {
        const hints = [];
        ...
        if (typeof window !== 'undefined') {
            if (window.location.protocol === 'https:' &&
                ...
                !config.useProxy) {
                hints.push('The page is running over HTTPS but the MCP server is HTTP. Browsers often block this as mixed content; enable the proxy or use HTTPS/WSS for the MCP server.');
            }
            if (window.location.origin !== targetUrl.origin && !config.useProxy) {
                hints.push('This is a cross-origin browser request. If the server is reachable from curl or Node but not from the browser, missing CORS headers are the most likely cause.');
            }
        }
        if (headerNames.length > 0) {
            hints.push(`Custom request headers are configured (${headerNames.join(', ')}). That triggers a CORS preflight, so the server must allow OPTIONS and include the matching Access-Control-Allow-Headers response.`);
        }
        if (config.credentials && config.credentials !== 'omit') {
            hints.push('Credentials are enabled for this connection. Cross-origin credentialed requests need Access-Control-Allow-Credentials: true and cannot use a wildcard Access-Control-Allow-Origin.');
        }
        if (message.includes('Failed to fetch')) {
            hints.push('"Failed to fetch" is a browser-level network failure. Common causes are CORS rejection, mixed-content blocking, certificate/TLS errors, DNS failures, or nothing listening on the target port.');
        }
        return hints;
    }
    static createDiagnosticFetch(serverName, config, baseInit, targetUrl, useProxy, onLog) {
        let enabled = true;
        ...
        return {
            fetch: async (input, init) => {
                if (useProxy && typeof window !== 'undefined') {
                    ...
                    if (requestUrlStr) {
                        ...
                        if (parsedRequestUrl.origin === window.location.origin &&
                            ...
                            const realTargetUrl = new URL(parsedRequestUrl.pathname + parsedRequestUrl.search, originalConfigUrl.origin);
                ...
                const request = this.createDiagnosticRequestDetails(input, init, baseInit, requestHeaders, Object.keys(config.headers ?? {}));
                const { method, url } = request;
                logIfEnabled(this.createLog(MCPConnectionPhase.INITIALIZING, `HTTP ${method} ${url}`, MCPLogLevel.INFO, {
                    serverName,
                    request
                }));
                if (method === 'DELETE' && url.includes(CORS_PROXY_ENDPOINT)) {
                    const response = new Response(null, { status: 200, statusText: 'OK' });
                    logIfEnabled(this.createLog(MCPConnectionPhase.INITIALIZING, `HTTP 200 ${method} ${url} (fake response)`, MCPLogLevel.INFO, {
                        response: {
                            url,
                ...
                try {
                    ...
                    logIfEnabled(this.createLog(MCPConnectionPhase.INITIALIZING, `HTTP ${response.status} ${method} ${url} (${durationMs}ms)`, response.ok ? MCPLogLevel.INFO : MCPLogLevel.WARN, {
                        response: {
                            url,
                            status: response.status,
                            statusText: response.statusText,
                    ...
                    const durationMs = Math.round(performance.now() - startedAt);
                    logIfEnabled(this.createLog(MCPConnectionPhase.ERROR, `HTTP ${method} ${url} failed: ${formatDiagnosticErrorMessage(error)}`, MCPLogLevel.ERROR, {
                        serverName,
                        request,
     ...
     */
    static createTransport(serverName, config, onLog) {
        if (!config.url) {
        ...
        if (config.transport === MCPTransportType.WEBSOCKET) {
            if (useProxy) {
                throw new Error('WebSocket transport is not supported when using CORS proxy. Use HTTP transport instead.');
            }
        ...
        if (config.transport === MCPTransportType.SSE) {
            ...
            const { fetch: diagnosticFetch, disable: stopPhaseLogging } = this.createDiagnosticFetch(serverName, config, requestInit, url, useProxy, onLog);
            if (import.meta.env.DEV && import.meta.env.VITE_DEBUG) {
     ...
     */
    static async connect(serverName, serverConfig, clientInfo, capabilities, onPhase, listChangedHandlers) {
        ...
        onPhase?.(MCPConnectionPhase.TRANSPORT_CREATING, this.createLog(MCPConnectionPhase.TRANSPORT_CREATING, `Creating transport for ${serverConfig.url}`));
        if (import.meta.env.DEV && import.meta.env.VITE_DEBUG) {
            console.log(`[MCPService][${serverName}] Creating transport...`);
        }
        const { transport, type: transportType, stopPhaseLogging } = this.createTransport(serverName, serverConfig, (log) => onPhase?.(log.phase, log));
        ...
        if (transportType === MCPTransportType.WEBSOCKET) {
            transport.onclose = () => {
                ...
                onPhase?.(MCPConnectionPhase.DISCONNECTED, this.createLog(MCPConnectionPhase.DISCONNECTED, 'WebSocket connection closed'));
            };
        }
        // Phase: Transport ready
        onPhase?.(MCPConnectionPhase.TRANSPORT_READY, this.createLog(MCPConnectionPhase.TRANSPORT_READY, `Transport ready (${transportType})`), { transportType });
        ...
        const runtimeErrorHandler = (error) => {
            ...
            if (/SSE stream disconnected:.*AbortError/i.test(msg) ||
        ...
        client.onerror = (error) => {
            onPhase?.(MCPConnectionPhase.ERROR, this.createLog(MCPConnectionPhase.ERROR, `Protocol error: ${error.message}`, MCPLogLevel.ERROR, {
                error: this.summarizeError(error)
            }));
        };
        // Phase: Initializing
        onPhase?.(MCPConnectionPhase.INITIALIZING, this.createLog(MCPConnectionPhase.INITIALIZING, 'Sending initialize request...'));
        ...
        const handshakeTimeoutMs = serverConfig.handshakeTimeoutMs ?? DEFAULT_MCP_CONFIG.connectionTimeoutMs;
        try {
            let handshakeTimer;
            const handshakeDeadline = new Promise((_, reject) => {
            ...
            client.onerror = runtimeErrorHandler;
            const url = (serverConfig.useProxy ?? false)
                ? buildProxiedUrl(serverConfig.url)
                : new URL(serverConfig.url);
            onPhase?.(MCPConnectionPhase.ERROR, this.createLog(MCPConnectionPhase.ERROR, `Connection failed during initialize: ${error instanceof Error ? error.message : String(error)}`, MCPLogLevel.ERROR, {
                error: this.summarizeError(error),
                config: {
                    serverName,
                    ...
                    headers: sanitizeHeaders(serverConfig.headers, Object.keys(serverConfig.headers ?? {}), MCP_PARTIAL_REDACT_HEADERS),
        ...
        onPhase?.(MCPConnectionPhase.CAPABILITIES_EXCHANGED, this.createLog(MCPConnectionPhase.CAPABILITIES_EXCHANGED, 'Capabilities exchanged successfully', MCPLogLevel.INFO, {
            serverCapabilities,
            serverInfo
        }), {
            serverInfo,
        ...
        // Phase: Listing tools
        onPhase?.(MCPConnectionPhase.LISTING_TOOLS, this.createLog(MCPConnectionPhase.LISTING_TOOLS, 'Listing available tools...'));
        if (import.meta.env.DEV && import.meta.env.VITE_DEBUG) {
        ...
        const tools = await this.listTools({
            ...
            transportType,
            connectionTimeMs: 0,
            requestTimeoutMs: serverConfig.requestTimeoutMs ?? DEFAULT_MCP_CONFIG.requestTimeoutSeconds * 1000
        });
        const connectionTimeMs = Math.round(performance.now() - startTime);
        // Phase: Connected
        onPhase?.(MCPConnectionPhase.CONNECTED, this.createLog(MCPConnectionPhase.CONNECTED, `Connection established with ${tools.length} tools (${connectionTimeMs}ms)`));
        if (import.meta.env.DEV && import.meta.env.VITE_DEBUG) {
            console.log(`[MCPService][${serverName}] Initialization complete with ${tools.length} tools in ${connectionTimeMs}ms`);
        }
        return {
            client,
            transport,
    ...
    static async callTool(connection, params, signal) {
        ...
        try {
            const result = await connection.client.callTool({ name: params.name, arguments: params.arguments }, undefined, { signal, timeout: connection.requestTimeoutMs });
            return {
            ...
            const message = error instanceof Error ? error.message : String(error);
            throw new Error(`Tool "${params.name}" execution failed on server "${connection.serverName}": ${message}`, { cause: error instanceof Error ? error : undefined });
        }
    }
    /**
    ...
    static formatSingleContent(content) {
     ...
     */
    static async complete(connection, ref, argument) {
        try {
    ...
    static async listResourceTemplates(connection, cursor) {
        try {
            ...
            console.warn(`[MCPService][${connection.serverName}] Failed to list resource templates:`, error);
     ...
     */
    static async readResource(connection, uri) {
        try {
     ...
     */
    static async subscribeResource(connection, uri) {
        try {
            await connection.client.subscribeResource({ uri });
            console.log(`[MCPService][${connection.serverName}] Subscribed to resource: ${uri}`);
        }
        catch (error) {
            console.error(`[MCPService][${connection.serverName}] Failed to subscribe to resource:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    static async unsubscribeResource(connection, uri) {
        try {
            ...
            console.error(`[MCPService][${connection.serverName}] Failed to unsubscribe from resource:`, error);
            throw error;
        }
    }
    /**

...
import { STORAGE_APP_NAME, STORAGE_APP_NAME_DEPRECATED, DB_APP_NAME_DEPRECATED, CONFIG_LOCALSTORAGE_KEY, IDXDB_TABLES, IDXDB_STORES, NEW_TO_DEPRECATED_MAP } from '$lib/constants';
...
function isMigrationCompleted(id) {
    const state = getMigrationState();
    return state.completed.includes(id);
}
function markMigrationCompleted(id) {
    const state = getMigrationState();
...
function markMigrationFailed(id) {
    const state = getMigrationState();
    ...
    async run() {
        ...
        for (const [newKey, deprecatedKey] of Object.entries(NEW_TO_DEPRECATED_MAP)) {
            ...
            if (oldValue !== null) {
                ...
                if (import.meta.env.DEV && import.meta.env.VITE_DEBUG) {
                    console.log(`[Migration] localStorage: copied ${deprecatedKey} → ${newKey} (preserved old)`);
                }
            }
        }
    }
...
function parseLegacyToolCalls(content) {
    ...
    let lastIndex = 0;
    let currentTurn = { textBefore: '', toolCalls: [] };
    let match;
...
function extractLegacyReasoning(content) {
    let reasoning = '';
    ...
    async run() {
        ...
        for (const conv of conversations) {
            ...
            for (const message of allMessages) {
                ...
                if (!hasLegacyMarkers(message.content ?? ''))
                    continue;
                const { reasoning, cleanContent } = extractLegacyReasoning(message.content);
                ...
                const firstTurnToolCalls = firstTurn.toolCalls.map((tc, i) => {
                    const existing = existingToolCalls.find((e) => e.function?.name === tc.name) || existingToolCalls[i];
                    return {
...
const customJsonKeyMigration = {
    ...
    async run() {
        ...
        if (!('custom' in config))
            return;
        if (SETTINGS_KEYS.CUSTOM_JSON in config)
            return;
...
const mcpDefaultEnabledMigration = {
    id: MCP_DEFAULT_ENABLED_MIGRATION_ID,
    description: 'Copy mcpDefaultEnabled localStorage key into settings config (preserves legacy keys)',
    async run() {
        const raw = localStorage.getItem(LEGACY_MCP_DEFAULT_ENABLED_KEY) ??
        ...
        try {
            ...
            if (!Array.isArray(parsed))
                return;
            const valid = parsed.every((o) => typeof o === 'object' &&
                o !== null &&
 ...
 */
const mcpDefaultOverridesMergeMigration = {
    id: MCP_DEFAULT_OVERRIDES_MERGE_MIGRATION_ID,
    description: 'Merge mcpDefaultServerOverrides entries onto mcpServers[i].enabled (preserves legacy key)',
    async run() {
        const configRaw = localStorage.getItem(CONFIG_LOCALSTORAGE_KEY);
        if (configRaw === null)
            return;
        ...
        try {
            ...
            if (!Array.isArray(parsed))
                return;
            overrides = parsed.filter((o) => typeof o === 'object' &&
                o !== null &&
                typeof o.serverId === 'string' &&
                typeof o.enabled === 'boolean');
        }
        ...
        for (const override of overrides) {
            if (!knownIds.has(override.serverId))
                continue;
        ...
        if (import.meta.env.DEV && import.meta.env.VITE_DEBUG)
            console.log(`[Migration] MCP default overrides merge: applied=${overrides.length} serversChanged=${serversChanged} (legacy key preserved)`);
    }
};
const migrations = [

...
import { MODEL_QUANTIZATION_SEGMENT_RE, MODEL_CUSTOM_QUANTIZATION_PREFIX_RE, MODEL_PARAMS_RE, MODEL_ACTIVATED_PARAMS_RE, MODEL_IGNORED_SEGMENTS, MODEL_WEIGHT_EXTENSION_RE, MODEL_ID_NOT_FOUND, MODEL_ID_ORG_SEPARATOR, MODEL_ID_SEGMENT_SEPARATOR, MODEL_ID_QUANTIZATION_SEPARATOR, API_MODELS } from '$lib/constants';
export class ModelsService {
     ...
     */
    static async list() {
        return apiFetch(API_MODELS.LIST);
    }
    /**
     ...
     */
    static async load(modelId, extraArgs) {
        const payload = { model: modelId };
        if (extraArgs && extraArgs.length > 0) {
            payload.extra_args = extraArgs;
        }
        return apiPost(API_MODELS.LOAD, payload);
    }
    /**
     ...
     */
    static async unload(modelId) {
        return apiPost(API_MODELS.UNLOAD, { model: modelId });
    }
    /**
     *
     *

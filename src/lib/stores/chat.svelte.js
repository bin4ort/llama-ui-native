...
import { selectedModelName, modelsStore, selectedModelContextSize } from '$lib/stores/models.svelte';
import { normalizeModelName, filterByLeafNodeId, findDescendantMessages, findLeafNode, findMessageById, isAbortError, generateConversationTitle } from '$lib/utils';
import { classifyContinueIntent } from '$lib/utils/agentic';
import { MAX_INACTIVE_CONVERSATION_STATES, INACTIVE_CONVERSATION_STATE_MAX_AGE_MS, SYSTEM_MESSAGE_PLACEHOLDER, TITLE_GENERATION } from '$lib/constants';
import { ContinueIntentKind, ErrorDialogType, MessageRole, MessageType, ReasoningEffort, StreamConnectionState } from '$lib/enums';
class ChatStore {
    ...
    chatReasoningStates = new SvelteMap();
    ...
    remoteRunningConvs = new SvelteSet();
    ...
    attachingConvs = new SvelteSet();
    // in-flight discoverActiveStream guard, keyed by conv id
    discoveringConvs = new SvelteSet();
    ...
    isStreamingActive = $state(false);
    isEditModeActive = $state(false);
    addFilesHandler = $state(null);
    pendingEditMessageId = $state(null);
    messageUpdateCallback = null;
    _pendingDraftMessage = $state('');
    _pendingDraftFiles = $state([]);
    /** Reactive: queued pending messages for non-agentic streaming */
    _pendingMessages = new SvelteMap();
    ...
    setChatReasoning(convId, reasoning) {
        if (reasoning) {
    ...
    setChatStreaming(convId, response, messageId, model) {
        this.touchConversationState(convId);
    ...
    clearChatStreaming(convId, messageId) {
     ...
     */
    async probeServerStream(convId) {
        if (!convId)
            return null;
        let listResp;
        try {
    ...
    async attachServerStream(convId, streamId) {
        if (!convId)
            return;
        ...
        if (targetIdx === -1) {
            ...
            if (lastUserIdx === -1) {
                console.warn(`attachServerStream: conv ${convId} has no user or assistant message, cannot splice`);
                unlock();
                return;
            }
            try {
        ...
        const attachedModel = sepIdx === -1 ? null : id.slice(sepIdx + 2);
        ...
        try {
            await ChatService.handleStreamResponse(response, (chunk) => {
            ...
            }, async (finalContent, reasoningContent, timings, toolCalls) => {
            ...
            }, undefined, undefined, undefined, undefined, convId, abortController.signal, (connState) => {
                if (convId === conversationsStore.activeConversation?.id) {
    ...
    async discoverActiveStream(convId) {
        if (!convId)
            return;
        if (this.chatStreamingStates.has(convId))
            return;
        if (this.chatLoadingStates.get(convId))
            return;
        ...
        if (this.discoveringConvs.has(convId))
            return;
    ...
    setProcessingState(conversationId, state) {
        if (state === null)
    ...
    getAllLoadingChats() {
        ...
        const out = new SvelteSet(this.chatLoadingStates.keys());
     ...
     */
    async syncRemoteRunningStreams() {
        ...
        const lookupIds = ids.map((id) => ChatService.resumeStreamIdentity(id, ChatService.getStreamState(id), null));
        let sessions;
        try {
    ...
    cleanupOldConversationStates(activeConversationIds) {
        ...
        for (const convId of allConvIds) {
            if (preserveIds.includes(convId))
                continue;
            if (this.chatLoadingStates.get(convId))
                continue;
            if (this.chatStreamingStates.has(convId))
        ...
        for (const { convId, lastAccessed } of cleanupCandidates) {
            if (cleanupCandidates.length - cleanedUp > MAX_INACTIVE_CONVERSATION_STATES ||
    ...
    async addSystemPrompt() {
        ...
        try {
            ...
            const existingSystemMessage = allMessages.find((m) => m.role === MessageRole.SYSTEM && m.parent === rootId);
            ...
            const systemMessage = await DatabaseService.createSystemMessage(activeConv.id, SYSTEM_MESSAGE_PLACEHOLDER, rootId);
            if (firstActiveMessage) {
                ...
                const updatedRootChildren = rootMessage
                    ? rootMessage.children.filter((id) => id !== firstActiveMessage.id)
                    : [];
                await DatabaseService.updateMessage(rootId, {
                    children: [
                        ...updatedRootChildren.filter((id) => id !== systemMessage.id),
                        systemMessage.id
                    ]
                });
    ...
    async removeSystemPromptPlaceholder(messageId) {
        ...
        try {
            ...
            await DatabaseService.updateMessage(rootMessage.id, {
                children: [
                    ...rootMessage.children.filter((id) => id !== messageId),
    ...
    async createAssistantMessage(parentId) {
        const activeConv = conversationsStore.activeConversation;
        if (!activeConv)
            throw new Error('No active conversation');
    ...
    async sendMessage(content, extras) {
        ...
        try {
            ...
            if (isNewConversation) {
                ...
                if (systemPrompt) {
                    const systemMessage = await DatabaseService.createSystemMessage(currentConv.id, systemPrompt, rootId);
            ...
            const userMessage = await this.addMessage(MessageRole.USER, content, MessageType.TEXT, parentIdForUserMessage ?? '-1', allExtras);
            if (isNewConversation && content)
                await conversationsStore.updateConversationName(currentConv.id, generateConversationTitle(content, Boolean(config().titleGenerationUseFirstLine)));
            const assistantMessage = await this.createAssistantMessage(userMessage.id);
            conversationsStore.addMessageToActive(assistantMessage);
            await this.streamChatCompletion(conversationsStore.activeMessages.slice(0, -1), assistantMessage, undefined, undefined, undefined, config().titleGenerationUseLLM && isNewConversation ? content : undefined);
        }
        catch (error) {
            ...
            this.setChatLoading(currentConv.id, false);
            const dialogType = error instanceof Error && error.name === 'TimeoutError'
                ? ErrorDialogType.TIMEOUT
                : ErrorDialogType.SERVER;
            const contextInfo = error.contextInfo;
    ...
    async streamChatCompletion(allMessages, assistantMessage, onComplete, onError, modelOverride, firstUserMessageContent) {
        ...
        const recordModel = (modelName, persistImmediately = true) => {
            if (!modelName)
                return;
            const n = normalizeModelName(modelName);
            if (!n || n === resolvedModel)
                return;
            resolvedModel = n;
        ...
        const recordCompletionId = (id) => {
            if (!id || completionIdRecorded)
                return;
        ...
        const streamCallbacks = {
            ...
            onCompletionId: (id) => recordCompletionId(id),
            ...
            onTimings: (timings, promptProgress) => {
                const tokensPerSecond = timings?.predicted_ms && timings?.predicted_n
            ...
            onAssistantTurnComplete: async (content, reasoningContent, timings, toolCalls) => {
                const updateData = {
                    content,
                ...
                if (timings)
                    uiUpdate.timings = timings;
                if (resolvedModel)
                    uiUpdate.model = resolvedModel;
            ...
            createToolResultMessage: async (toolCallId, content, extras) => {
            ...
            updateToolResultMessage: async (messageId, content, extras) => {
            ...
            onFlowComplete: (finalTimings) => {
                ...
                if (config().preEncodeConversation) {
                    this.triggerPreEncode(allMessages, assistantMessage, streamedContent, effectiveModel, !!config().excludeReasoningFromContext);
                }
            },
            onError: async (error) => {
                this.setStreamingActive(false);
        ...
        await ChatService.sendMessage(allMessages, {
            ...
            onComplete: async (finalContent, reasoningContent, timings, toolCalls) => {
    ...
    async generateTitleWithLLM(userContent, assistantContent, convId) {
        const effectiveModel = isRouterMode() && selectedModelName() ? selectedModelName() : undefined;
        const configValue = config();
        const titlePromptTemplate = typeof configValue.titleGenerationPrompt === 'string' &&
    ...
    async savePartialResponseIfNeeded(convId) {
        ...
        if (!streamingState)
            return;
        const messages = conversationId === conversationsStore.activeConversation?.id
        ...
        try {
            ...
            if (partialContent.trim())
                updateData.content = partialContent;
    ...
    async updateMessage(messageId, newContent) {
        ...
        try {
            ...
            if (isFirstUserMessage && newContent.trim())
                await conversationsStore.updateConversationName(activeConv.id, generateConversationTitle(newContent, Boolean(config().titleGenerationUseFirstLine)));
            ...
            await this.streamChatCompletion(conversationsStore.activeMessages.slice(0, -1), assistantMessage, undefined, () => {
    ...
    async regenerateMessage(messageId) {
        ...
        try {
            ...
            this.setChatLoading(activeConv.id, true);
            this.clearChatStreaming(activeConv.id);
            const parentMessageId = conversationsStore.activeMessages.length > 0
            ...
            conversationsStore.addMessageToActive(assistantMessage);
            await this.streamChatCompletion(conversationsStore.activeMessages.slice(0, -1), assistantMessage);
        }
        catch (error) {
            if (!isAbortError(error))
    ...
    async regenerateMessageWithBranching(messageId, modelOverride) {
        ...
        try {
            ...
            const conversationPath = filterByLeafNodeId(allMessages, parentMessage.id, false);
            const modelToUse = modelOverride || msg.model || undefined;
            await this.streamChatCompletion(conversationPath, newAssistantMessage, undefined, undefined, modelToUse);
        }
        catch (error) {
            if (!isAbortError(error))
        ...
        if (messageToDelete?.role === MessageRole.SYSTEM) {
            ...
            for (const msg of messagesToDelete) {
                if (msg.role === MessageRole.USER) {
                    userMessages++;
                    if (!messageTypes.includes('user message'))
        ...
        try {
            ...
            if (isInCurrentPath && messageToDelete.parent) {
                ...
                if (siblings.length > 0) {
                    const latestSibling = siblings.reduce((latest, sibling) => sibling.timestamp > latest.timestamp ? sibling : latest);
                    await conversationsStore.updateCurrentNode(findLeafNode(allMessages, latestSibling.id));
                }
                else if (messageToDelete.parent) {
                    await conversationsStore.updateCurrentNode(findLeafNode(allMessages, messageToDelete.parent));
                }
            }
     ...
     */
    async continueAsNextAgenticTurn(anchorIndex) {
        const activeConv = conversationsStore.activeConversation;
        if (!activeConv)
            return;
        ...
        try {
            ...
            conversationsStore.updateConversationTimestamp();
            await conversationsStore.refreshActiveMessages();
            const conversationPath = filterByLeafNodeId(allMessages, anchorMessage.id, false);
            await this.streamChatCompletion(conversationPath, newAssistantMessage);
        }
        catch (error) {
            if (!isAbortError(error))
                console.error('Failed to continue agentic turn:', error);
            this.setChatLoading(activeConv.id, false);
        }
    }
    async continueAssistantMessage(messageId) {
        ...
        try {
            ...
            await ChatService.sendMessage(contextWithContinue, {
                ...this.getApiOptions(),
            ...
            }, msg.convId, abortController.signal);
        }
        catch (error) {
            if (!isAbortError(error))
    ...
    async editMessageWithBranching(messageId, newContent, newExtras) {
        ...
        try {
            const allMessages = await conversationsStore.getConversationMessages(activeConv.id);
            const rootMessage = allMessages.find((m) => m.type === 'root' && m.parent === null);
            const isFirstUserMessage = msg.role === MessageRole.USER && rootMessage && msg.parent === rootMessage.id;
            const extrasToUse = newExtras !== undefined
    ...
    async generateResponseForMessage(userMessageId) {
        ...
        try {
            const allMessages = await conversationsStore.getConversationMessages(activeConv.id);
            const conversationPath = filterByLeafNodeId(allMessages, userMessageId, false);
    ...
    parseTimingData(timingData) {
        const promptTokens = timingData.prompt_n || 0, promptMs = timingData.prompt_ms || undefined, predictedTokens = timingData.predicted_n || 0, tokensPerSecond = timingData.predicted_per_second || 0, cacheTokens = timingData.cache_n || 0;
        const promptProgress = timingData.prompt_progress;
        const contextTotal = this.getContextTotal();
        const currentConfig = config();
        const outputTokensMax = currentConfig.max_tokens || -1;
        const contextUsed = promptTokens + cacheTokens + predictedTokens, outputTokensUsed = predictedTokens;
        const progressCache = promptProgress?.cache || 0, progressActualDone = (promptProgress?.processed ?? 0) - progressCache, progressActualTotal = (promptProgress?.total ?? 0) - progressCache;
    ...
    getApiOptions() {
        ...
        const apiOptions = { stream: true, timings_per_token: true };
        if (isRouterMode()) {
            const modelName = selectedModelName();
            if (modelName)
                apiOptions.model = modelName;
        }
        if (currentConfig.systemMessage)
            apiOptions.systemMessage = currentConfig.systemMessage;
        if (currentConfig.disableReasoningParsing)
        ...
        if (effort !== ReasoningEffort.DEFAULT) {
            ...
            if (effort !== ReasoningEffort.OFF)
                apiOptions.reasoningEffort = effort;
        }
        ...
        if (currentConfig.customJson)
            apiOptions.custom = currentConfig.customJson;
        return apiOptions;
    }
    cancelPreEncode() {
    ...
    async triggerPreEncode(allMessages, assistantMessage, assistantContent, model, excludeReasoning) {
        this.cancelPreEncode();
...
export const getChatStreaming = (convId) => chatStore.getChatStreamingPublic(convId);
export const isChatLoading = (convId) => chatStore.isChatLoadingPublic(convId);
...
export const chatHasPendingMessage = (convId) => chatStore.hasPendingMessage(convId);
export const chatPendingMessageContent = (convId) => chatStore.pendingMessageContent(convId);
export const chatPendingMessageExtras = (convId) => chatStore.pendingMessageExtras(convId);
export const chatClearPendingMessage = (convId) => chatStore.clearPendingMessage(convId);
export const chatInjectPendingMessage = (convId, content, extras) => chatStore.injectPendingMessage(convId, content, extras);

...
import { CONTEXT_GAUGE_CARD_HALF_WIDTH_PX, CONTEXT_GAUGE_CLOSE_GRACE_MS, CONTEXT_GAUGE_DIAL_GAP_PX, CONTEXT_GAUGE_EDGE_MARGIN_PX } from '$lib/constants';
let closeTimer;
let lastPointerType = '';
...
export function gaugePopupClose() {
    clearTimeout(closeTimer);
    gaugePopup.open = false;
}
export function gaugeTriggerPointerDown(event) {
    lastPointerType = event.pointerType;
}
export function gaugeTriggerClick(event) {
    if (lastPointerType !== 'touch')
        return;
    toggleFrom(event.currentTarget);
}
export function gaugeTriggerKeydown(event) {
    if (event.key !== 'Enter' && event.key !== ' ')
        return;
    event.preventDefault();
    toggleFrom(event.currentTarget);
}
export function gaugeTriggerEnter(event) {
    if (event.pointerType !== 'mouse')
        return;
    openFrom(event.currentTarget);
}
export function gaugeTriggerLeave(event) {
    if (event.pointerType !== 'mouse')
        return;
    scheduleClose();
}
export function gaugeCardEnter(event) {
    if (event.pointerType !== 'mouse')
        return;
    clearTimeout(closeTimer);
}
export function gaugeCardLeave(event) {
    if (event.pointerType !== 'mouse')
        return;
    scheduleClose();
}

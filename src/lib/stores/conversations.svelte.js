...
import { MessageRole, HtmlInputType, FileExtensionText, MimeTypeText, MimeTypeApplication, ReasoningEffort } from '$lib/enums';
import { ISO_DATE_TIME_SEPARATOR, ISO_DATE_TIME_SEPARATOR_REPLACEMENT, ISO_TIMESTAMP_SLICE_LENGTH, EXPORT_CONV_ID_TRIM_LENGTH, EXPORT_CONV_NONALNUM_REPLACEMENT, EXPORT_CONV_NAME_SUFFIX_MAX_LENGTH, ISO_TIME_SEPARATOR, ISO_TIME_SEPARATOR_REPLACEMENT, NON_ALPHANUMERIC_REGEX, MULTIPLE_UNDERSCORE_REGEX, REASONING_EFFORT_DEFAULT_LOCALSTORAGE_KEY } from '$lib/constants';
...
class ConversationsStore {
     ...
     */
    registerMessageUpdateCallback(callback) {
     ...
     */
    updateMessageAtIndex(index, updates) {
     ...
     */
    sliceActiveMessages(startIndex) {
    ...
    async loadConversation(convId) {
        try {
            ...
            if (conversation.currNode) {
                ...
                const filteredMessages = filterByLeafNodeId(allMessages, conversation.currNode, false);
     ...
     */
    async deleteConversation(convId, options) {
        try {
            ...
            if (options?.deleteWithForks) {
                ...
                this.conversations = this.conversations
                    ...
                    .map((c) => c.forkedFromConversationId === convId
     ...
     */
    async bulkDeleteConversations(convIds) {
        if (convIds.length === 0)
            return;
        try {
            ...
            const activeWasDeleted = this.activeConversation !== null && idsToRemove.has(this.activeConversation.id);
            ...
            toast.success(convIds.length === 1 ? 'Conversation deleted' : `${convIds.length} conversations deleted`);
        }
        catch (error) {
        ...
        try {
            ...
            for (let i = 0; i < this.conversations.length; i++) {
                ...
                if (newPinned !== undefined)
                    this.conversations[i].pinned = newPinned;
            }
            this.conversations = [...this.conversations];
            toast.success(convIds.length === 1
     ...
     */
    async bulkExportConversations(convIds) {
        if (convIds.length === 0)
            return;
        try {
            ...
            toast.success(exported.length === 1
    ...
    async refreshActiveMessages() {
        ...
        const currentPath = filterByLeafNodeId(allMessages, leafNodeId, false);
     ...
     */
    async updateConversationName(convId, name) {
        try {
     ...
     */
    async toggleConversationPin(convId) {
        try {
     ...
     */
    async updateCurrentNode(nodeId) {
        if (!this.activeConversation)
            return;
     ...
     */
    async navigateToSibling(siblingId) {
        if (!this.activeConversation)
            return;
        const allMessages = await DatabaseService.getConversationMessages(this.activeConversation.id);
        const rootMessage = allMessages.find((m) => m.type === 'root' && m.parent === null);
        const currentFirstUserMessage = this.activeMessages.find((m) => m.role === MessageRole.USER && m.parent === rootMessage?.id);
        ...
        if (rootMessage && this.activeMessages.length > 0) {
            const newFirstUserMessage = this.activeMessages.find((m) => m.role === MessageRole.USER && m.parent === rootMessage.id);
            if (newFirstUserMessage &&
                newFirstUserMessage.content.trim() &&
                (!currentFirstUserMessage ||
                    newFirstUserMessage.id !== currentFirstUserMessage.id ||
                    newFirstUserMessage.content.trim() !== currentFirstUserMessage.content.trim())) {
                await this.updateConversationName(this.activeConversation.id, generateConversationTitle(newFirstUserMessage.content, Boolean(config().titleGenerationUseFirstLine)));
            }
        }
    }
    /**
     ...
     */
    getMcpServerOverride(serverId) {
        const override = this.activeConversation?.mcpServerOverrides?.find((o) => o.serverId === serverId);
        if (override)
            return override;
    ...
    getAllMcpServerOverrides() {
        ...
        return mcpStore.getServers().map((s) => {
            const override = overrides?.find((o) => o.serverId === s.id);
     ...
     */
    isMcpServerEnabledForChat(serverId) {
     ...
     */
    async setMcpServerOverride(serverId, enabled) {
        if (!this.activeConversation) {
        ...
        const currentOverrides = (this.activeConversation.mcpServerOverrides || []).map((o) => ({
            serverId: o.serverId,
            enabled: o.enabled
        }));
        let newOverrides;
        if (enabled === undefined) {
            newOverrides = currentOverrides.filter((o) => o.serverId !== serverId);
        }
        else {
            const existingIndex = currentOverrides.findIndex((o) => o.serverId === serverId);
            if (existingIndex >= 0) {
     ...
     */
    async removeMcpServerOverride(serverId) {
     ...
     */
    async forkConversation(messageId, options) {
        if (!this.activeConversation)
            return null;
        try {
            const newConv = await DatabaseService.forkConversation(this.activeConversation.id, messageId, options);
     ...
     */
    generateConversationFilename(conversation, msgs) {
    ...
    async parseImportFile(file) {
        ...
        if (name.endsWith(FileExtensionText.ZIP)) {
            ...
            for (const [entryName, bytes] of Object.entries(entries)) {
                if (!entryName.toLowerCase().endsWith(FileExtensionText.JSONL))
                    continue;
     ...
     */
    downloadConversationFile(data, filename) {
     ...
     */
    downloadConversationsArchive(data) {
        if (data.length === 0) {
            console.error('Invalid data: no conversations to export');
            return;
        }
        const usedNames = new SvelteSet();
        const files = {};
    ...
    async downloadConversation(convId) {
        const conversation = this.activeConversation?.id === convId
            ? this.activeConversation
        ...
        return new Promise((resolve, reject) => {
            ...
            input.onchange = async (e) => {
                ...
                try {
                    ...
                    await this.loadConversations();
                    const importedConversations = (Array.isArray(importedData) ? importedData : [importedData]).map((item) => item.conv);
     ...
     */
    async importConversationsData(data) {
...
const comparePinnedThenRecent = (a, b) => {
...
export function buildConversationTree(convs) {
    const childrenByParent = new SvelteMap();
    const forkIds = new SvelteSet();

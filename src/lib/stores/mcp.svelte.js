...
import { parseMcpServerSettings, detectMcpTransportFromUrl, uuid, extractRootDomain } from '$lib/utils';
import { MCPConnectionPhase, MCPLogLevel, HealthCheckStatus, MCPRefType, ColorMode, UrlProtocol } from '$lib/enums';
import { DEFAULT_CACHE_TTL_MS, DEFAULT_MCP_CONFIG, EXPECTED_THEMED_ICON_PAIR_COUNT, MCP_ALLOWED_ICON_MIME_TYPES, MCP_SERVER_ID_PREFIX, MCP_RECONNECT_BACKOFF_MULTIPLIER, MCP_RECONNECT_INITIAL_DELAY, MCP_RECONNECT_MAX_DELAY, MCP_RECONNECT_ATTEMPT_TIMEOUT_MS } from '$lib/constants';
class MCPStore {
    _isInitializing = $state(false);
    _error = $state(null);
    _toolCount = $state(0);
    _connectedServers = $state([]);
    _healthChecks = $state({});
    connections = new Map();
    toolsIndex = new Map();
    serverConfigs = new Map(); // Store configs for reconnection
    reconnectingServers = new Set(); // Guard against concurrent reconnections
    ...
    #parseServerSettings(rawServers) {
        ...
        return parsed.map((entry, index) => {
            ...
            return {
                id: this.#generateServerId(entry?.id, index),
     ...
     */
    #requestTimeoutMs() {
        const seconds = Number(config().mcpRequestTimeoutSeconds) || DEFAULT_MCP_CONFIG.requestTimeoutSeconds;
        return Math.round(seconds * 1000);
    }
    /**
     * Builds server configuration from a settings entry.
     */
    #buildServerConfig(entry, connectionTimeoutMs = DEFAULT_MCP_CONFIG.connectionTimeoutMs) {
        if (!entry?.url) {
            return undefined;
        }
        let headers;
    ...
    #buildMcpClientConfig(cfg, perChatOverrides) {
        ...
        for (const [index, entry] of rawServers.entries()) {
            if (!this.#checkServerEnabled(entry, perChatOverrides))
                continue;
            const normalized = this.#buildServerConfig(entry);
            if (normalized)
                servers[this.#generateServerId(entry.id, index)] = normalized;
        }
    ...
    updateHealthCheck(serverId, state) {
    ...
    hasHealthCheck(serverId) {
        return (serverId in this._healthChecks &&
     ...
     */
    #serverBaseLabel(server) {
        if (server.displayName)
            return server.displayName;
        const healthState = this.getHealthCheckState(server.id);
        if (healthState?.status === HealthCheckStatus.SUCCESS)
            return (healthState.serverInfo?.title || healthState.serverInfo?.name || server.name || server.url);
     ...
     */
    #getMcpIconUrl(icons, isDark = false) {
        if (!icons?.length)
            return null;
        const validIcons = icons.filter((icon) => {
            if (!icon.src || !this.#isValidIconUri(icon.src))
                return false;
            if (icon.mimeType && !MCP_ALLOWED_ICON_MIME_TYPES.has(icon.mimeType))
                return false;
            return true;
        });
     ...
     */
    #getServerFaviconFallback(serverUrl) {
        try {
    ...
    updateServer(id, updates) {
        const servers = this.getServers();
        settingsStore.updateConfig(SETTINGS_KEYS.MCP_SERVERS, JSON.stringify(servers.map((server) => (server.id === id ? { ...server, ...updates } : server))));
    }
    removeServer(id) {
        const servers = this.getServers();
        settingsStore.updateConfig(SETTINGS_KEYS.MCP_SERVERS, JSON.stringify(servers.filter((s) => s.id !== id)));
    ...
    async doInitialize(signature, mcpConfig, serverEntries) {
        ...
        const results = await Promise.allSettled(serverEntries.map(async ([name, serverConfig]) => {
            ...
            const connection = await MCPService.connect(name, serverConfig, clientInfo, capabilities, (phase) => {
            ...
            if (result.status === 'fulfilled') {
                ...
                for (const tool of connection.tools) {
                    if (this.toolsIndex.has(tool.name))
                        console.warn(`[MCPStore] Tool name conflict: "${tool.name}" exists in "${this.toolsIndex.get(tool.name)}" and "${name}". Using tool from "${name}".`);
    ...
    handleToolsListChanged(serverName, tools) {
        ...
        for (const tool of tools) {
            if (this.toolsIndex.has(tool.name))
                console.warn(`[MCPStore] Tool name conflict after list change: "${tool.name}" exists in "${this.toolsIndex.get(tool.name)}" and "${serverName}". Using tool from "${serverName}".`);
     ...
     */
    async releaseConnection(shutdownIfUnused = false) {
    ...
    async shutdown() {
        ...
        await Promise.all(Array.from(this.connections.values()).map((conn) => MCPService.disconnect(conn).catch((error) => console.warn(`[MCPStore] Error disconnecting ${conn.serverName}:`, error))));
    ...
    async reconnectServer(serverName) {
        ...
        const connection = await MCPService.connect(serverName, serverConfig, DEFAULT_MCP_CONFIG.clientInfo, DEFAULT_MCP_CONFIG.capabilities, (phase) => {
            if (phase === MCPConnectionPhase.DISCONNECTED) {
        ...
        try {
            while (true) {
                ...
                try {
                    ...
                    const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`Reconnect attempt timed out after ${MCP_RECONNECT_ATTEMPT_TIMEOUT_MS}ms`)), MCP_RECONNECT_ATTEMPT_TIMEOUT_MS));
                    needsReconnect = false;
                    const listChangedHandlers = this.createListChangedHandlers(serverName);
                    const connectPromise = MCPService.connect(serverName, serverConfig, DEFAULT_MCP_CONFIG.clientInfo, DEFAULT_MCP_CONFIG.capabilities, (phase) => {
                        if (phase === MCPConnectionPhase.DISCONNECTED) {
                            if (this.reconnectingServers.has(serverName)) {
            ...
            if (needsReconnect) {
                console.log(`[MCPStore][${serverName}] Deferred disconnect detected, restarting auto-reconnect`);
                this.autoReconnect(serverName);
            }
        }
    }
    ...
    findServerForTool(toolName) {
        ...
        for (const server of this.getServers()) {
            ...
            if (!health || health.status !== HealthCheckStatus.SUCCESS)
                continue;
     ...
     */
    getServerFaviconForTool(toolName) {
        if (!toolName)
            return null;
    ...
    hasPromptsCapability(perChatOverrides) {
        ...
        if (perChatOverrides !== undefined) {
            ...
            enabledServerIds = new Set(this.getServers()
                .filter((s) => s.enabled)
                .map((s) => s.id));
        }
        ...
        for (const [serverName, connection] of this.connections) {
            if (!enabledServerIds.has(serverName))
                continue;
            if (connection.serverCapabilities?.prompts) {
                return true;
            }
    ...
    parseToolArguments(args) {
        if (typeof args === 'string') {
            ...
            try {
                ...
                if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed))
                    throw new Error(`Tool arguments must be an object, got ${Array.isArray(parsed) ? 'array' : typeof parsed}`);
                return parsed;
            }
            catch (error) {
                throw new Error(`Failed to parse tool arguments as JSON: ${error.message}`);
            }
        }
    ...
    async getPromptCompletions(serverName, promptName, argumentName, argumentValue) {
        const connection = this.connections.get(serverName);
        if (!connection) {
        ...
        return MCPService.complete(connection, { type: MCPRefType.PROMPT, name: promptName }, { name: argumentName, value: argumentValue });
    }
    /**
     * Get completions for a resource template argument.
     * Uses the MCP Completion API with ref/resource.
     */
    async getResourceCompletions(serverName, uriTemplate, argumentName, argumentValue) {
        const connection = this.connections.get(serverName);
        if (!connection) {
            console.warn(`[MCPStore] Server "${serverName}" is not connected`);
            return null;
        ...
        return MCPService.complete(connection, { type: MCPRefType.RESOURCE, uri: uriTemplate }, { name: argumentName, value: argumentValue });
    }
    /**
     ...
     */
    async readResourceByUri(serverName, uri) {
        const connection = this.connections.get(serverName);
        if (!connection) {
    ...
    async runHealthChecksForServers(servers, skipIfChecked = true, promoteToActive = false) {
     ...
     */
    async runHealthCheck(server, promoteToActive = false) {
        ...
        if (existingConnection) {
            ...
            try {
                ...
                const capabilities = this.#buildCapabilitiesInfo(existingConnection.serverCapabilities, existingConnection.clientCapabilities);
                ...
                console.warn(`[MCPStore] Failed to reuse connection for ${server.id}, creating new one:`, error);
        ...
        try {
            ...
            const connection = await MCPService.connect(server.id, serverConfig, DEFAULT_MCP_CONFIG.clientInfo, DEFAULT_MCP_CONFIG.capabilities, (phase, log) => {
                currentPhase = phase;
                logs.push(log);
                this.updateHealthCheck(server.id, {
                    status: HealthCheckStatus.CONNECTING,
                ...
                if (phase === MCPConnectionPhase.DISCONNECTED && promoteToActive) {
                    console.log(`[MCPStore][${server.id}] Connection lost during health check, starting auto-reconnect`);
            ...
            const capabilities = this.#buildCapabilitiesInfo(connection.serverCapabilities, connection.clientCapabilities);
            this.updateHealthCheck(server.id, {
                status: HealthCheckStatus.SUCCESS,
                tools,
    ...
    promoteHealthCheckToConnection(serverId, connection) {
        ...
        for (const tool of connection.tools) {
            if (this.toolsIndex.has(tool.name)) {
                console.warn(`[MCPStore] Tool name conflict during promotion: "${tool.name}" exists in "${this.toolsIndex.get(tool.name)}" and "${serverId}". Using tool from "${serverId}".`);
            }
    ...
    getServersWithResources() {
        const enabledServerIds = new Set(this.getServers()
            .filter((s) => s.enabled)
            .map((s) => s.id));
        const servers = [];
        // Check active connections
        for (const [name, connection] of this.connections) {
            if (!enabledServerIds.has(name))
                continue;
     ...
     */
    async fetchAllResources(forceRefresh = false) {
        ...
        try {
            await Promise.all(serversWithResources.map((serverName) => this.fetchServerResources(serverName)));
        }
        finally {

...
import { detectThinkingSupport, detectThinkingSupportWithReason } from '$lib/utils/chat-template-thinking-detector';
import { TTLCache, getAuthHeaders } from '$lib/utils';
import { MODEL_PROPS_CACHE_TTL_MS, MODEL_PROPS_CACHE_MAX_ENTRIES, FAVORITE_MODELS_LOCALSTORAGE_KEY, API_MODELS, SSE_RECORD_SEPARATOR, SSE_LINE_SEPARATOR, SSE_DATA_PREFIX } from '$lib/constants';
...
class ModelsStore {
    ...
    inflightFetch = null;
    modelUsage = $state(new Map());
    modelLoadingStates = new SvelteMap();
    // /models/sse feed state, the single source of truth for status and load progress
    statusAbort = null;
    statusReaderActive = false;
    loadProgress = new SvelteMap();
    statusWaiters = new Map();
    favoriteModelIds = $state(this.loadFavoritesFromStorage());
    /**
    ...
    isModelOperationInProgress(modelId) {
    ...
    getModelUsage(modelId) {
        return this.modelUsage.get(modelId) ?? new SvelteSet();
    }
     ...
     */
    checkModelSupportsThinking(modelId) {
        if (!isRouterMode()) {
            return detectThinkingSupport(serverStore.props?.chat_template ?? '');
        }
        if (!modelId)
     ...
     */
    buildModelOptions(response) {
        return response.data.map((item, index) => {
            const details = response.models?.[index];
            const rawCapabilities = Array.isArray(details?.capabilities) ? details?.capabilities : [];
            const displayNameSource = details?.name && details.name.trim().length > 0 ? details.name : item.id;
            ...
            return {
                ...
                capabilities: rawCapabilities.filter((value) => Boolean(value)),
    ...
    async fetchModelProps(modelId) {
        ...
        if (this.modelPropsFetching.has(modelId))
            return null;
    ...
    async fetchModalitiesForLoadedModels() {
     ...
     */
    async updateModelModalities(modelId) {
        const props = await this.fetchModelProps(modelId);
        if (!props?.modalities)
            return;
        this.models = this.models.map((model) => model.model === modelId
     ...
     */
    getModelFromLastAssistantResponse() {
     ...
     */
    async selectModelFromLastAssistantResponse() {
        const lastModel = this.getModelFromLastAssistantResponse();
        if (!lastModel || this.selectedModelName === lastModel)
            return false;
        const matchingModel = this.models.find((option) => option.model === lastModel);
        if (!matchingModel || !this.isModelLoaded(lastModel))
            return false;
        try {
     ...
     */
    async ensureFirstModelSelected() {
        if (this.selectedModelName)
            return;
    ...
    findModelByName(modelName) {
        return (this.models.find((model) => model.model === modelName || model.id === modelName || model.aliases?.includes(modelName)) ?? null);
    }
    ...
    applyModelStatus(event) {
        ...
        if (status === ServerModelStatus.LOADING) {
            if (data.progress)
                this.loadProgress.set(model, data.progress);
        }
        else {
        ...
        const failed = status === ServerModelStatus.FAILED ||
    ...
    removeRouterModel(modelId) {
        if (this.routerModels.findIndex((m) => m.id === modelId) === -1)
            return;
     ...
     */
    setRouterModelStatus(modelId, status) {
    ...
    async loadModel(modelId) {
        if (this.isModelLoaded(modelId))
            return;
        if (this.modelLoadingStates.get(modelId))
            return;
...
export const checkModelSupportsThinking = (modelId) => modelsStore.checkModelSupportsThinking(modelId);

...
import { CONFIG_LOCALSTORAGE_KEY, SETTING_CONFIG_DEFAULT, SETTINGS_KEYS, USER_OVERRIDES_LOCALSTORAGE_KEY } from '$lib/constants';
import { isMobile } from '$lib/stores/viewport.svelte';
import { ParameterSyncService } from '$lib/services/parameter-sync.service';
import { serverStore } from '$lib/stores/server.svelte';
import { configToParameterRecord, normalizeFloatingPoint, getConfigValue, setConfigValue } from '$lib/utils';
class SettingsStore {
    /**
     *
     *
    ...
    initialize() {
        try {
            ...
            setMode(normalizeThemeForSetMode(this.config[SETTINGS_KEYS.THEME]));
            applyThemeClass(this.config[SETTINGS_KEYS.THEME]);
    ...
    loadConfig() {
        ...
        try {
            ...
            // Load user overrides
            const savedOverrides = JSON.parse(localStorage.getItem(USER_OVERRIDES_LOCALSTORAGE_KEY) || '[]');
    ...
    saveConfig() {
        ...
        try {
            ...
            localStorage.setItem(USER_OVERRIDES_LOCALSTORAGE_KEY, JSON.stringify(Array.from(this.userOverrides)));
        }
        catch (error) {
     ...
     */
    resetParameterToServerDefault(key) {
            ...
            for (const [key, value] of Object.entries(uiSettings)) {
                if (!this.userOverrides.has(key) && value !== undefined) {
                    ...
                    if (key === SETTINGS_KEYS.THEME) {
                        setMode(normalizeThemeForSetMode(value));
                    }
                }
            }
        }
    ...
    forceSyncWithServerDefaults() {
        ...
        for (const key of Object.keys(SETTING_CONFIG_DEFAULT)) {
            ...
            const value = uiSettings && key in uiSettings && uiSettings[key] !== undefined
                ? uiSettings[key]
    ...
    canSyncParameter(key) {
        return ParameterSyncService.canSyncParameter(key);
    }
    /**
     * Get parameter information including source for a specific parameter
     */
    getParameterInfo(key) {
        const propsDefaults = this.getServerDefaults();
        const currentValue = getConfigValue(this.config, key);
        return ParameterSyncService.getParameterInfo(key, currentValue ?? '', propsDefaults, this.userOverrides);
    }
    /**
    ...
    getParameterDiff() {
        ...
        if (Object.keys(serverDefaults).length === 0)
            return {};
        const configAsRecord = configToParameterRecord(this.config, ParameterSyncService.getSyncableParameterKeys());
     ...
     */
    exportSettings(includeSensitiveData = false) {
        // Build config excluding sensitive data unless user opts in
        const configToExport = includeSensitiveData
            ? { ...this.config }
        ...
        if ('mcpServers' in configToExport && !includeSensitiveData) {
            try {
                const mcpServers = JSON.parse(configToExport.mcpServers);
                ...
                delete configToExport.mcpServers;
            }
        }
        return {
            version: 1,
    ...
    /**
     * Auto-detect the machine's network IP for Web UI Address.
     * Called once on initialization if the address is still the default.
     */
    async detectNetworkAddress() {
        if (!browser)
            return;
        const current = this.config[SETTINGS_KEYS.WEB_UI_ADDRESS];
        if (current && current !== 'http://localhost:8765')
            return;
        try {
            const resp = await fetch('/api/network-info');
            if (resp.ok) {
                const data = await resp.json();
                this.updateConfig(SETTINGS_KEYS.WEB_UI_ADDRESS, data.url);
            }
        }
        catch {
            // Network detection failed — keep default
        }
    }
}

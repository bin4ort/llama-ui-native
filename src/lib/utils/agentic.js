import { AgenticSectionType, AttachmentType, ContinueIntentKind, MessageRole, ToolResultKind } from '$lib/enums';
import { ATTACHMENT_SAVED_REGEX, MARKDOWN_ATX_HEADING_REGEX, MARKDOWN_BOLD_REGEX, MARKDOWN_BLOCKQUOTE_REGEX, MARKDOWN_CODE_FENCE_REGEX, MARKDOWN_LINK_REGEX, MARKDOWN_LIST_BULLET_REGEX, MARKDOWN_LIST_NUMBERED_REGEX, MARKDOWN_TABLE_SEPARATOR_REGEX, NEWLINE, REASONING_TAGS, SEARCH_SUMMARY_SEPARATOR, SEARCH_SUMMARY_TOTAL_REGEX, TOOL_RESULT_JSON_OPEN_REGEX } from '$lib/constants';
/**
 ...
 */
function deriveSingleTurnSections(message, toolMessages = [], streamingToolCalls = [], isStreaming = false) {
    const sections = [];
    // 1. Reasoning content (from dedicated field)
    if (message.reasoningContent) {
        const toolCalls = parseToolCalls(message.toolCalls);
        const hasContentAfterReasoning = !!message.content?.trim() || toolCalls.length > 0 || streamingToolCalls.length > 0;
 ...
 */
export function deriveAgenticSections(message, toolMessages = [], streamingToolCalls = [], isStreaming = false) {
    ...
    while (i < toolMessages.length) {
        ...
        if (msg.role === MessageRole.ASSISTANT) {
            ...
            sections.push(...deriveSingleTurnSections(msg, turnToolMsgs, isLastTurn ? streamingToolCalls : [], isLastTurn && isStreaming));
 ...
 */
export function buildAssistantRawOutput(sections) {
    const parts = [];
    for (const section of sections) {
        switch (section.type) {
            ...
            case AgenticSectionType.TOOL_CALL_STREAMING: {
                const callObj = { name: section.toolName };
                if (section.toolArgs) {
                    try {
 ...
 */
export function splitSearchSummaryList(text, captureTotal) {
    ...
    const summaryText = separatorIndex === -1 ? '' : text.slice(separatorIndex + SEARCH_SUMMARY_SEPARATOR.length);
 ...
 */
export function parseToolResultWithImages(toolResult, extras) {
    ...
    return lines.map((line) => {
        ...
        const image = extras.find((e) => e.type === AttachmentType.IMAGE && e.name === attachmentName);
...
export function classifyToolResult(content) {
    ...
    if (looksLikeMarkdown(trimmed))
        return ToolResultKind.MARKDOWN;
    return ToolResultKind.TEXT;
}
/**
...
function looksLikeMarkdown(content) {
    ...
    if (MARKDOWN_CODE_FENCE_REGEX.test(content))
        return true;
    const lines = content.split(NEWLINE);
    for (const line of lines) {
        if (MARKDOWN_ATX_HEADING_REGEX.test(line))
            return true;
        if (MARKDOWN_BLOCKQUOTE_REGEX.test(line))
            return true;
        if (MARKDOWN_LIST_BULLET_REGEX.test(line))
            return true;
        if (MARKDOWN_LIST_NUMBERED_REGEX.test(line))
            return true;
    }
    // Inline structural markers anywhere in the body.
    if (MARKDOWN_LINK_REGEX.test(content))
        return true;
    if (MARKDOWN_BOLD_REGEX.test(content))
        return true;
    // Tables: a pipe-bearing header line followed by a separator row.
    if (lines.length >= 2) {
        const head = lines[0];
        const sep = lines[1];
        if (head.includes('|') && MARKDOWN_TABLE_SEPARATOR_REGEX.test(sep))
            return true;
    }
    return false;
}
 ...
 */
export function hasAgenticContent(message, toolMessages = []) {
 ...
 */
export function classifyContinueIntent(messages, idx) {

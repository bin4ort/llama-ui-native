...
export async function apiFetch(path, options = {}) {
    ...
    const url = path.startsWith(UrlProtocol.HTTP) || path.startsWith(UrlProtocol.HTTPS)
        ? path
        : `${getApiBase()}${path}`;
    let response;
    try {
 ...
 * ```
 */
export async function apiFetchWithParams(basePath, params, options = {}) {
    const url = new URL(basePath, getApiBase());
 ...
 */
function beautifyNetworkError(throwable) {
    let message;

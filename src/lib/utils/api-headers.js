...
import { AUTHORIZATION_HEADER, BEARER_PREFIX, CONTENT_TYPE_HEADER, CORS_PROXY_HEADER_PREFIX, REDACTED_HEADERS } from '$lib/constants';
 ...
 */
export function sanitizeHeaders(headers, extraRedactedHeaders, partialRedactHeaders) {
    if (!headers) {
        return {};
    }
    const normalized = new Headers(headers);
    const sanitized = {};
    const redactedHeaders = new Set(Array.from(extraRedactedHeaders ?? [], (header) => header.toLowerCase()));
    for (const [key, value] of normalized.entries()) {
        ...
        const unproxiedKey = normalizedKey.startsWith(CORS_PROXY_HEADER_PREFIX)
            ...
            : normalizedKey;
        const partialChars = partialRedactHeaders?.get(normalizedKey) ?? partialRedactHeaders?.get(unproxiedKey);
        if (partialChars !== undefined) {
            sanitized[key] = redactValue(value, partialChars);
        }
        else if (REDACTED_HEADERS.has(normalizedKey) ||

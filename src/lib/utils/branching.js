 ...
 */
export function filterByLeafNodeId(messages, leafNodeId, includeRoot = false) {
    const result = [];
 ...
 */
function findLeafNodeInMap(nodeMap, messageId) {
 ...
 */
export function findLeafNode(messages, messageId) {
    const nodeMap = new Map(messages.map((msg) => [msg.id, msg]));
 ...
 */
export function findDescendantMessages(messages, messageId) {
    const nodeMap = new Map();
    // Build node map for quick lookups
    for (const msg of messages) {
        nodeMap.set(msg.id, msg);
 ...
 */
export function getMessageSiblings(nodeMap, messageId) {
    ...
    const siblingLeafIds = siblingIds.map((siblingId) => findLeafNodeInMap(nodeMap, siblingId));

...
const JINJA_THINKING_CONDITIONALS = [
 ...
 */
export function detectThinkingSupport(t) {
    if (!t)
        return false;
    for (const kwarg of THINKING_KWARG_VARS) {
        const regex = new RegExp(`(\\{\\{[^{}]*\\b${kwarg}\\b[^{}]*\\}\\}|\\{%[^{}]*\\b${kwarg}\\b[^{}]*%\\})`, 'i');
        if (regex.test(t))
            return true;
    }
...
export function detectThinkingSupportWithReason(t) {
    if (!t)
        return { supported: false, reason: 'No chat template available' };
    for (const kwarg of THINKING_KWARG_VARS) {
        const regex = new RegExp(`(\\{\\{[^{}]*\\b${kwarg}\\b[^{}]*\\}\\}|\\{%[^{}]*\\b${kwarg}\\b[^{}]*%\\})`, 'i');
        if (regex.test(t)) {
    ...
    for (const p of JINJA_THINKING_CONDITIONALS) {
        if (p.test(t))
            return { supported: true, reason: 'Found: thinking conditional' };
    }
    for (const [s, e] of THINKING_TAG_PATTERNS) {
        if (t.includes(s) && (!e || t.includes(e))) {

 ...
 * @returns Promise<boolean> - True if successful, false otherwise
 */
export async function copyCodeToClipboard(rawCode, successMessage = 'Code copied to clipboard', errorMessage = 'Failed to copy code') {
 ...
 */
export function formatMessageForClipboard(content, extras, asPlainText = false) {
    // Filter text-like attachments (TEXT, LEGACY_CONTEXT, MCP_PROMPT, and MCP_RESOURCE types)
    const textAttachments = extras?.filter((extra) => extra.type === AttachmentType.TEXT ||
 ...
 */
export function parseClipboardContent(clipboardText) {
    const defaultResult = {
    ...
    try {
        ...
        const message = JSON.parse(jsonStringPart);
        ...
        const validMcpPromptAttachments = [];
...
function isValidMcpPromptAttachment(obj) {
    ...
    const record = obj;
    return ((record.type === AttachmentType.MCP_PROMPT || record.type === 'MCP_PROMPT') &&
...
function isValidTextAttachment(obj) {
    ...
    const record = obj;
    return ((record.type === AttachmentType.TEXT || record.type === 'TEXT') &&
        typeof record.name === 'string' &&
        typeof record.content === 'string');
}
/**
 ...
 */
export function hasClipboardAttachments(clipboardText) {
    if (!clipboardText.startsWith('"')) {
        return false;
    }

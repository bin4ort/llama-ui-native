import { EXIF_SCAN_BYTE_LIMIT, JPEG_SOI_MARKER, APP1_MARKER, SOS_MARKER, EXIF_SIGNATURE, TIFF_LITTLE_ENDIAN, TIFF_MAGIC, EXIF_ORIENTATION_TAG, IFD_ENTRY_SIZE } from '$lib/constants/jpeg-exif';
 ...
 */
export function getJpegOrientationFromDataURL(base64UrlJpeg) {
    try {
 ...
 */
function findExifOrientation(view) {
 ...
 * @returns The orientation value (1 to 8), or 1 when absent or malformed
 */
function parseExifOrientation(view, start, segmentLength) {
 ...
 */
export function isJpegMimeType(mimeType) {

 ...
 */
function hasLegacyMarkers(message) {
    if (!message.content)
        return false;
 ...
 */
async function migrateConversation(convId) {
        ...
        if (message.children.length > 0 && currentParentId !== message.id) {
            for (const childId of message.children) {
                ...
                if (child.role !== MessageRole.TOOL) {
                    ...
                    const newParent = await DatabaseService.getConversationMessages(convId).then((msgs) => msgs.find((m) => m.id === currentParentId));
...
export async function runLegacyMigration() {
    ...
    try {
        ...
        if (import.meta.env.DEV && import.meta.env.VITE_DEBUG) {
            if (totalMigrated > 0) {
                console.log(`[Migration] Migrated ${totalMigrated} messages across ${conversations.length} conversations`);
            }
            else {

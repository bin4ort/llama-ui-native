...
export async function convertPDFToText(file) {
    ...
    try {
        ...
        const textItems = textContents.flatMap((textContent) => textContent.items.map((item) => item.str ?? ''));
 ...
 */
export async function convertPDFToImage(file, scale = 1.5) {
    if (!browser) {
        throw new Error('PDF processing is only available in the browser');
    }
    try {
 ...
 */
export function isApplicationMimeType(mimeType) {

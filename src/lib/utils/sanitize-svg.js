...
export function sanitizeSvg(source) {
    ...
    if (!trimmed || trimmed.length > SVG_MAX_BYTES)
        return '';
    if (!trimmed.startsWith(SVG_TAG_PREFIX))
        return '';
    const clean = DOMPurify.sanitize(trimmed, SVG_SANITIZE_CONFIG);
    if (!clean || !clean.includes(SVG_TAG_PREFIX))
        return '';
    return clean;
}

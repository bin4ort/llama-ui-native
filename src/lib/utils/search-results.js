...
function parseChunk(chunk) {
    ...
    if (fields[FieldKey.PUBLISHED])
        result.published = fields[FieldKey.PUBLISHED];
    if (fields[FieldKey.AUTHOR])
        result.author = fields[FieldKey.AUTHOR];
    if (highlights)
 ...
 * block.
 */
export function extractSearchResults(text) {
    if (!text)
        return [];
    const results = [];
 ...
 */
export function extractSearchQuery(toolArgs) {
    if (!toolArgs)
        return '';
    try {
        const parsed = JSON.parse(toolArgs);
        if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
            const candidate = parsed[SEARCH_TOOL_QUERY_FIELD];
...
export function faviconForUrl(url) {
    try {
        ...
        if (!RESOLVABLE_URL_PROTOCOLS.includes(parsed.protocol))
            return null;
...
export const SUPPORTED_WEB_SEARCH_TOOL_NAMES = ['web_search_exa'];
/**
 ...
 */
export function isWebSearchToolName(toolName) {
    if (!toolName)
        return false;

 ...
 */
export function isTextFileByName(filename) {
 ...
 */
export function isLikelyTextFile(content, options = {}) {
    if (!content)
        return true;
    ...
    if (nullCount > config.maxAbsoluteNullBytes)
        return false;
    // Reject if too many suspicious characters
    if (suspiciousControlCount / sample.length > config.suspiciousCharThresholdRatio)
        return false;
    return true;
}

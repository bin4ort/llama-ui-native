import { TEMPLATE_EXPRESSION_REGEX, URI_SCHEME_SEPARATOR, URI_TEMPLATE_OPERATORS, URI_TEMPLATE_SEPARATORS, VARIABLE_EXPLODE_MODIFIER_REGEX, VARIABLE_PREFIX_MODIFIER_REGEX, LEADING_SLASHES_REGEX } from '../constants';
/**
...
export function expandTemplate(template, values) {
    TEMPLATE_EXPRESSION_REGEX.lastIndex = 0;
    return template.replace(TEMPLATE_EXPRESSION_REGEX, (_match, operator, varList) => {
        ...
        switch (operator) {
            ...
            case URI_TEMPLATE_OPERATORS.FRAGMENT:
                // Fragment expansion
                return (URI_TEMPLATE_OPERATORS.FRAGMENT + expandedParts.join(URI_TEMPLATE_SEPARATORS.COMMA));
            ...
            case URI_TEMPLATE_OPERATORS.LABEL:
                // Label expansion
                return (URI_TEMPLATE_SEPARATORS.PERIOD + expandedParts.join(URI_TEMPLATE_SEPARATORS.PERIOD));
            case URI_TEMPLATE_OPERATORS.PATH_PARAM:
                // Path-style parameters
                return varNames
                    .filter((_, i) => expandedParts[i])
                    .map((name, i) => `${URI_TEMPLATE_SEPARATORS.SEMICOLON}${name}=${expandedParts[i]}`)
                    .join('');
            case URI_TEMPLATE_OPERATORS.FORM_QUERY:
                // Form-style query
                return (URI_TEMPLATE_SEPARATORS.QUERY_PREFIX +
                    varNames
                        .filter((_, i) => expandedParts[i])
                        .map((name, i) => `${encodeURIComponent(name)}=${encodeURIComponent(expandedParts[i])}`)
                        .join(URI_TEMPLATE_SEPARATORS.QUERY_CONTINUATION));
            case URI_TEMPLATE_OPERATORS.FORM_CONTINUATION:
                // Form-style query continuation
                return (URI_TEMPLATE_SEPARATORS.QUERY_CONTINUATION +
                    varNames
                        .filter((_, i) => expandedParts[i])
                        .map((name, i) => `${encodeURIComponent(name)}=${encodeURIComponent(expandedParts[i])}`)
 ...
 */
export function isTemplateComplete(template, values) {

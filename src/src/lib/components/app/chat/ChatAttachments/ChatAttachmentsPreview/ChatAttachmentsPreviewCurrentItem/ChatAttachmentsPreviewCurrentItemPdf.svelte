...
{#if !hasVisionModality && activeModelId && currentItem}
	<Alert.Root class="mb-4 max-w-4xl">
		...
		<Alert.Title>{tr.dict["Preview only"] || "Preview only"}</Alert.Title>
		<Alert.Description>
...
{#if pdfImagesLoading}
	<div class="flex flex-1 items-center justify-center p-8">
		<div class="text-center">
			...
			></div>
			<p class="text-white/70">{tr.dict["Converting PDF to images..."] || "Converting PDF to images..."}</p>
		</div>
	</div>
{:else if pdfImagesError}
	<div class="flex flex-1 items-center justify-center p-8">
		<div class="text-center">
			...
			<p class="mb-4 text-white/70">{tr.dict["Failed to load PDF images"] || "Failed to load PDF images"}</p>
...
{:else}
	<div class="flex flex-1 items-center justify-center p-8">
		<div class="text-center">
			<FileText class="mx-auto mb-4 h-16 w-16 text-white/50" />
			<p class="text-white/70">{tr.dict["No PDF pages available"] || "No PDF pages available"}</p>
		</div>
	</div>
{/if}

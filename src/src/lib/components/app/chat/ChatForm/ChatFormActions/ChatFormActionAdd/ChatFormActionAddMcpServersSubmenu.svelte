...
<DropdownMenu.Root>
	<DropdownMenu.Sub onOpenChange={handleMcpSubMenuOpen}>
		<DropdownMenu.SubTrigger class="flex cursor-pointer items-center gap-2">
			...
			<span>{tr.dict["MCP Servers"] || "MCP Servers"}</span>
		</DropdownMenu.SubTrigger>

		<DropdownMenu.SubContent class="w-72 pt-0">
			{#if hasMcpServers}
				<DropdownMenuSearchable
					placeholder={tr.dict["Search servers..."] || "Search servers..."}
					bind:searchValue={mcpSearchQuery}
					emptyMessage={tr.dict["No servers found"] || "No servers found"}
				...
				>
					...
					{#snippet footer()}
						...
						>
							...
							<span>{tr.dict["Manage MCP Servers"] || "Manage MCP Servers"}</span>
						</DropdownMenu.Item>
					{/snippet}
				</DropdownMenuSearchable>
			{:else}
				...
				>
					...
					<span>{tr.dict["Add MCP Servers"] || "Add MCP Servers"}</span>
				</DropdownMenu.Item>
			{/if}

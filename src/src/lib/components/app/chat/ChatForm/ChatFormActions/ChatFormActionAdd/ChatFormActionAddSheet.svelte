...
<div class="flex items-center gap-1 {className}">
	<Sheet.Root bind:open={sheetOpen}>
		...
		<Sheet.Content side="bottom" class="max-h-[85vh] gap-0 overflow-y-auto">
			<Sheet.Header>
				<Sheet.Title>{tr.dict["Add to chat"] || "Add to chat"}</Sheet.Title>

				<Sheet.Description class="sr-only">
					{tr.dict["Add files, system prompt or configure MCP servers"] || "Add files, system prompt or configure MCP servers"}
				</Sheet.Description>
			</Sheet.Header>
			...
			<div class="flex flex-col gap-1 px-1.5 pb-2">
				{#if reasoning.modelSupportsThinking}
					...
					>
						<Collapsible.Trigger class={sheetItemClass}>
							...
							<span class="flex-1">{tr.dict["Reasoning"] || "Reasoning"}</span>
				...
				<Collapsible.Root open={filesExpanded} onOpenChange={(open) => (filesExpanded = open)}>
					<Collapsible.Trigger class={sheetItemClass}>
						...
						<span class="flex-1">{tr.dict["Add files"] || "Add files"}</span>
					</Collapsible.Trigger>
				...
				<Collapsible.Root open={mcpExpanded} onOpenChange={(open) => (mcpExpanded = open)}>
					<Collapsible.Trigger class={sheetItemClass}>
						...
						<span class="flex-1">{tr.dict["MCP Servers"] || "MCP Servers"}</span>
						...
						<div class="flex flex-col gap-0.5 pl-4">
							...
							{#if mcpServers.length === 0}
								<div class="px-3 py-2 text-center text-sm text-muted-foreground">
									{tr.dict["No MCP servers configured"] || "No MCP servers configured"}
								</div>
							{/if}
						</div>
					</Collapsible.Content>
				...
				{#if toolsPanel.totalToolCount > 0}
					<Collapsible.Root open={toolsExpanded} onOpenChange={(open) => (toolsExpanded = open)}>
						<Collapsible.Trigger class={sheetItemClass}>
							...
							<span class="flex-1">{tr.dict["Tools"] || "Tools"}</span>
				...
				>
					...
					<span>{tr.dict["System Message"] || "System Message"}</span>
				</button>
				...
				{#if hasMcpPromptsSupport}
					...
					>
						...
						<span>{tr.dict["MCP Prompt"] || "MCP Prompt"}</span>
					</button>
				{/if}
				...
				{#if hasMcpResourcesSupport}
					...
					>
						...
						<span>{tr.dict["MCP Resources"] || "MCP Resources"}</span>
					</button>
				{/if}
			</div>
		</Sheet.Content>

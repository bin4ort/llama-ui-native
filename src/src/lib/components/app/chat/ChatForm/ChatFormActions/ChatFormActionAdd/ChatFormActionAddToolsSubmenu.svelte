		...
		{#if toolsPanel.totalToolCount === 0}
			{#if toolsStore.loading}
				<div class="px-3 py-4 text-center text-sm text-muted-foreground">
					...
					{tr.dict["Loading tools..."] || "Loading tools..."}
				</div>
			...
			{:else if toolsStore.error}
				<div class="px-3 py-4 text-center text-sm text-muted-foreground">{tr.dict["Failed to load tools"] || "Failed to load tools"}</div>
			...
			{:else}
				<div class="px-3 py-4 text-center text-sm text-muted-foreground">{tr.dict["No tools available"] || "No tools available"}</div>
			{/if}
		{:else}

...
<Collapsible.Root bind:open class="mt-3 border-t border-border/50 pt-4">
	...
	>
		<span>{tr.dict["Token usage details"] || "Token usage details"}</span>
	...
	<Collapsible.Content class="flex flex-col gap-4 text-xs pt-4">
		{#if hasCumulative}
			<div>
				<h3 class="text-[11px] font-medium uppercase tracking-wide text-muted-foreground/70 mb-2">
					{tr.dict["Across all turns"] || "Across all turns"}
				</h3>

				<div class="flex flex-col gap-2">
					{#if cumulativeRead > 0}
						<ContextGaugeDetailRow
							label={tr.dict["Prompt tokens evaluated"] || "Prompt tokens evaluated"}
					...
					{#if cumulativeOutput > 0}
						<ContextGaugeDetailRow
							label={tr.dict["Tokens generated"] || "Tokens generated"}
		...
		{#if hasCurrent}
			<div>
				...
				<div class="flex flex-col gap-2">
					...
					{#if currentOutput > 0}
						<ContextGaugeDetailRow
							label={tr.dict["Generated"] || "Generated"}
					...
					<div class="pt-1 mt-0.5 border-t border-border/30">
						<div class="flex justify-between">
							<span class="text-muted-foreground">{tr.dict["KV cache total"] || "KV cache total"}</span>
		...
		{#if averageTokensPerSecond !== null}
			<div class="pt-1.5 mt-1 border-t border-border/30">
				<ContextGaugeDetailRow
					label={tr.dict["Avg speed"] || "Avg speed"}

	...
	>
		<div class="flex flex-col gap-2">
			<div class="flex items-center gap-2">
				<span class="font-medium">{tr.dict["Context"] || "Context"}</span>
			...
			{:else if showProgressBar}
				...
				<div class="flex justify-between text-xs text-muted-foreground">
					<span>
						<span class={colorLevelTextClass(gauge.colorLevel)}>{gauge.contextPercent}%</span> {tr.dict["used"] || "used"}
					</span>
					<span>
						{formatParameters((gauge.contextTotal ?? 0) - gauge.contextUsed)} {tr.dict["remaining"] || "remaining"}
					</span>
				</div>
			{:else}
				<div class="text-xs text-muted-foreground">{tr.dict["No context info available"] || "No context info available"}</div>
			{/if}

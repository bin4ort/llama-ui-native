...
<form onsubmit={onSubmit} class="space-y-3 pt-4">
	...
	<div class="mt-8 flex justify-end gap-2">
		<Button type="button" size="sm" onclick={onCancel} variant="secondary">{tr.dict["Cancel"] || "Cancel"}</Button>

		<Button size="sm" type="submit">{tr.dict["Use Prompt"] || "Use Prompt"}</Button>
	</div>
</form>

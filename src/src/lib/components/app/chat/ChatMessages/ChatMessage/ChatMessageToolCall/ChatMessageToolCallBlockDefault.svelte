		...
		{#if ctx.isStreamingCall}
			...
			{:else if ctx.isStreaming}
				<div class="rounded bg-muted/20 p-2 text-xs text-muted-foreground/70 italic">
					{tr.dict["Receiving arguments..."] || "Receiving arguments..."}
				</div>
			{:else}
				<div
					class="rounded bg-yellow-500/10 p-2 text-xs text-yellow-600 italic dark:text-yellow-400"
				>
					{tr.dict["Response was truncated"] || "Response was truncated"}
				</div>
			{/if}
		{:else}
			...
			>
				<span>{tr.dict["Output"] || "Output"}</span>
				{#if ctx.isPending}
					<Loader2 class="h-3 w-3 animate-spin" />
				{/if}
			</div>
			{#if ctx.isPending}
				<div class="rounded bg-muted/20 p-2 text-xs text-muted-foreground/70 italic">
					{tr.dict["Waiting for result..."] || "Waiting for result..."}
				</div>
			{:else if section.toolResult}
				...
				{/if}
			{:else}
				<div class="rounded bg-muted/20 p-2 text-xs text-muted-foreground/70 italic">{tr.dict["No output"] || "No output"}</div>
			{/if}
		{/if}
	{/snippet}
</ToolCallBlock>

...
<ToolCallBlock {section} {open} {isStreaming} meta={editFileMeta} {onToggle}>
	{#snippet titleSnippet()}
		<span class="text-muted-foreground">{tr.dict["Edit file"] || "Edit file"} </span>
	...
	{#snippet children(meta, _ctx)}
		...
		{:else}
			<div class="rounded bg-muted/20 p-2 text-xs text-muted-foreground/70 italic">{tr.dict["No edits"] || "No edits"}</div>
		{/if}
	{/snippet}
</ToolCallBlock>

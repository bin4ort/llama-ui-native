...
<div class="text-muted-foreground flex items-center gap-2 py-1.5">
	...
	{#if showSpinner}
		<span class="text-foreground/80 text-sm font-medium">{tr.dict["Current time"] || "Current time"}</span>
	...
	{:else if dateMeta.dateString}
		<span class="text-foreground/80 text-sm font-medium">{tr.dict["Current time is"] || "Current time is"}&nbsp;</span>

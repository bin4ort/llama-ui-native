...
<ToolCallBlock {section} {open} {isStreaming} meta={grepMeta} {onToggle}>
	{#snippet titleSnippet()}
		{#if grepMeta}
			<span class="text-muted-foreground">{tr.dict["Search for"] || "Search for"}&nbsp;</span>
			<span class="font-mono">{grepMeta.pattern}</span>
			<span class="text-muted-foreground">&nbsp;{tr.dict["in"] || "in"}&nbsp;</span>
	...
	{#snippet children(meta, ctx)}
		...
		{:else}
			<div class="text-xs text-muted-foreground/70 italic">{tr.dict["No matches"] || "No matches"}</div>
			<div class="mt-1.5 text-xs text-muted-foreground/70 italic">

...
<div
	aria-label={tr.dict["Pending user message"] || "Pending user message"}
		...
		<div class="max-w-[80%]">
			<div class="relative flex h-6 items-center justify-between">
				<div class="right-0 flex items-center gap-2 opacity-100 transition-opacity">
					...
					>
						<ActionIcon icon={Edit} tooltip={tr.dict["Edit"] || "Edit"} onclick={editCtx.handleEdit} />
						<ActionIcon icon={Trash2} tooltip={tr.dict["Delete"] || "Delete"} onclick={onDelete} />
						<ActionIcon icon={ArrowUp} tooltip={tr.dict["Send immediately"] || "Send immediately"} onclick={onSendImmediately} />
					</div>
				</div>
			</div>
		</div>

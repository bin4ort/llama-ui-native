...
<div class="relative {justify === 'start' ? 'mt-2' : ''} flex h-6 items-center justify-between">
	...
	>
		...
		>
			<ActionIcon icon={Copy} tooltip={tr.dict["Copy"] || "Copy"} onclick={onCopy} />

			{#if onEdit}
				<ActionIcon icon={Edit} tooltip={tr.dict["Edit"] || "Edit"} onclick={onEdit} />
			{/if}

			{#if role === MessageRole.ASSISTANT && onRegenerate}
				<ActionIcon icon={RefreshCw} tooltip={tr.dict["Regenerate"] || "Regenerate"} onclick={() => onRegenerate()} />
			{/if}

			{#if role === MessageRole.ASSISTANT && onContinue}
				<ActionIcon icon={ArrowRight} tooltip={tr.dict["Continue"] || "Continue"} onclick={onContinue} />
			{/if}

			{#if onForkConversation}
				<ActionIcon icon={GitBranch} tooltip={tr.dict["Fork conversation"] || "Fork conversation"} onclick={handleOpenForkDialog} />
			{/if}

			<ActionIcon icon={Trash2} tooltip={tr.dict["Delete"] || "Delete"} onclick={onDelete} />
		</div>
	</div>

	{#if showRawOutputSwitch}
		<div class="flex items-center gap-2">
			<span class="text-xs text-muted-foreground">{tr.dict["Show raw output"] || "Show raw output"}</span>
			<Switch
...
<DialogConfirmation
	...
	title={tr.dict["Delete Message"] || "Delete Message"}
...
<DialogConfirmation
	...
	title={tr.dict["Fork Conversation"] || "Fork Conversation"}
	description={tr.dict["Create a new conversation branching from this message."] || "Create a new conversation branching from this message."}
	confirmText={tr.dict["Fork"] || "Fork"}
	cancelText={tr.dict["Cancel"] || "Cancel"}
	icon={GitBranch}
...
>
	<div class="flex flex-col gap-4 py-2">
		<div class="flex flex-col gap-2">
			<Label for="fork-name">{tr.dict["Title"] || "Title"}</Label>

			<Input
				id="fork-name"
				class="text-foreground"
				placeholder={tr.dict["Enter fork name"] || "Enter fork name"}
				type="text"
				bind:value={forkName}
			/>
		</div>
		...
		<div class="flex items-center gap-2">
			...
			<Label for="fork-attachments" class="cursor-pointer text-sm font-normal">
				{tr.dict["Include all attachments"] || "Include all attachments"}
			</Label>
		</div>
	</div>
</DialogConfirmation>

...
>
	<div class="mermaid-preview-controls-inner flex items-center gap-1 rounded-lg bg-muted p-1">
		<button
			...
			onclick={onZoomOut}
			title={tr.dict["Zoom out"] || "Zoom out"}
			aria-label={tr.dict["Zoom out"] || "Zoom out"}
		>
		...
		<button
			class="mermaid-preview-btn flex h-8 w-8 cursor-pointer items-center justify-center rounded-md border-0 bg-transparent text-foreground transition-colors hover:bg-muted-foreground/15 active:bg-muted-foreground/25"
			onclick={onZoomIn}
			title={tr.dict["Zoom in"] || "Zoom in"}
			aria-label={tr.dict["Zoom in"] || "Zoom in"}
		>
		...
		<button
			class="mermaid-preview-btn flex h-8 w-8 cursor-pointer items-center justify-center rounded-md border-0 bg-transparent text-foreground transition-colors hover:bg-muted-foreground/15 active:bg-muted-foreground/25"
			onclick={onResetView}
			title={tr.dict["Reset view"] || "Reset view"}
			aria-label={tr.dict["Reset view"] || "Reset view"}
		>
		...
		<button
			class="mermaid-preview-btn flex h-8 w-8 cursor-pointer items-center justify-center rounded-md border-0 bg-transparent text-foreground transition-colors hover:bg-muted-foreground/15 active:bg-muted-foreground/25"
			onclick={downloadSvg}
			title={tr.dict["Download SVG"] || "Download SVG"}
			aria-label={tr.dict["Download SVG"] || "Download SVG"}
		>

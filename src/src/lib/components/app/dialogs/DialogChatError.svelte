...
<AlertDialog.Root {open} onOpenChange={handleOpenChange}>
	<AlertDialog.Content>
		...
		<div class={`rounded-lg border px-4 py-3 text-sm ${badgeClass}`}>
			...
			{#if contextInfo}
				<div class="mt-2 space-y-1 text-xs opacity-80">
					<p>
						<span class="font-medium">{tr.dict["Prompt tokens:"] || "Prompt tokens:"}</span>
					...
					{#if contextInfo.n_ctx}
						<p>
							<span class="font-medium">{tr.dict["Context size:"] || "Context size:"}</span>
		...
		<AlertDialog.Footer>
			<AlertDialog.Action onclick={() => handleOpenChange(false)}>{tr.dict["Close"] || "Close"}</AlertDialog.Action>
		</AlertDialog.Footer>
	</AlertDialog.Content>
</AlertDialog.Root>

...
<Dialog.Root {open} onOpenChange={handleOpenChange}>
	<Dialog.Content class="max-h-[80vh] !max-w-4xl overflow-hidden p-0">
		<Dialog.Header class="border-b border-border/30 px-6 py-4">
			...
			<Dialog.Description>
				{tr.dict["Browse and attach resources from connected MCP servers to your chat context."] || "Browse and attach resources from connected MCP servers to your chat context."}
			</Dialog.Description>
		</Dialog.Header>
			...
			<div class="min-w-0 flex-1 overflow-auto p-4">
				...
				{:else}
					<div class="flex h-full items-center justify-center text-sm text-muted-foreground">
						{tr.dict["Select a resource to preview"] || "Select a resource to preview"}
					</div>
				{/if}
			</div>
		</div>
		...
		<Dialog.Footer class="border-t border-border/30 px-6 py-4">
			<Button variant="outline" onclick={() => handleOpenChange(false)}>{tr.dict["Cancel"] || "Cancel"}</Button>
			...
			{#if hasTemplateResult}
				<Button onclick={handleAttachTemplateResource} disabled={isAttaching}>
					...
					{tr.dict["Attach Resource"] || "Attach Resource"}
				</Button>
			{:else}
				<Button onclick={handleAttach} disabled={selectedResources.size === 0 || isAttaching}>
					...
					Attach {selectedResources.size > 0 ? `(${selectedResources.size})` : t("Resource")}
				</Button>
			{/if}
		</Dialog.Footer>
	</Dialog.Content>

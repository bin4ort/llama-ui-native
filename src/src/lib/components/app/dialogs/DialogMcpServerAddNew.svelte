...
<Dialog.Root {open} onOpenChange={handleOpenChange}>
	<Dialog.Content class="sm:max-w-2xl">
		<Dialog.Header>
			<Dialog.Title class="select-none">{tr.dict["Add New MCP Server"] || "Add New MCP Server"}</Dialog.Title>
		</Dialog.Header>

		{#if recommendationsToShow.length > 0}
			<div class="space-y-3 pt-2">
				<div class="flex items-center justify-between gap-3">
					<h3 class="text-sm font-medium">{tr.dict["Recommended Servers"] || "Recommended Servers"}</h3>
					<Button class="text-muted-foreground" variant="ghost" size="sm" onclick={handleDismissAll}
						>{tr.dict["Dismiss"] || "Dismiss"}</Button
					>
				</div>
		...
		<form onsubmit={handleSubmit} class="contents">
			...
			<Dialog.Footer>
				...
				<Button variant="default" size="sm" type="submit" disabled={!canSave} aria-label={tr.dict["Add"] || "Add"}>
					{tr.dict["Add"] || "Add"}
				</Button>
			</Dialog.Footer>
		</form>

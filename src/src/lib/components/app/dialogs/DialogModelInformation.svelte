...
<Dialog.Root bind:open {onOpenChange}>
	<Dialog.Content class="@container z-9999 !max-h-[80dvh] !max-w-[60rem] max-w-full">
		...
		<Dialog.Header>
			<Dialog.Title>{tr.dict["Model Information"] || "Model Information"}</Dialog.Title>

			<Dialog.Description>{tr.dict["Current model details and capabilities"] || "Current model details and capabilities"}</Dialog.Description>
		</Dialog.Header>

		<div class="space-y-6 py-4">
			{#if isLoadingModels || isLoadingRouterProps}
				<div class="flex items-center justify-center py-8">
					<div class="text-sm text-muted-foreground">{tr.dict["Loading model information..."] || "Loading model information..."}</div>
				</div>
			{:else if firstModel}
				...
				{#if serverProps}
					<Table.Root>
						<Table.Header>
							<Table.Row>
								<Table.Head class="w-[10rem]">{tr.dict["Model"] || "Model"}</Table.Head>
								...
								<Table.Head>
									<div class="inline-flex items-center gap-2">
										...
										<ActionIconCopyToClipboard
											text={modelName || ''}
											canCopy={!!modelName}
											ariaLabel={tr.dict["Copy model name to clipboard"] || "Copy model name to clipboard"}
										/>
									</div>
								</Table.Head>
							</Table.Row>
						...
						<Table.Body>
							<!-- Model Path -->
							<Table.Row>
								<Table.Cell class="h-10 align-middle font-medium">{tr.dict["File Path"] || "File Path"}</Table.Cell>
								...
								>
									...
									<ActionIconCopyToClipboard
										...
										ariaLabel={tr.dict["Copy model path to clipboard"] || "Copy model path to clipboard"}
									/>
								</Table.Cell>
							</Table.Row>
							...
							{#if serverProps?.default_generation_settings?.n_ctx}
								<Table.Row>
									<Table.Cell class="h-10 align-middle font-medium">{tr.dict["Context Size"] || "Context Size"}</Table.Cell>
							...
							{:else}
								<Table.Row>
									...
									<Table.Cell class="text-red-500">{tr.dict["Not available"] || "Not available"}</Table.Cell>
								</Table.Row>
							{/if}
							...
							{#if modelMeta?.n_ctx_train}
								<Table.Row>
									<Table.Cell class="h-10 align-middle font-medium">{tr.dict["Training Context"] || "Training Context"}</Table.Cell>
							...
							{#if modelMeta?.size}
								<Table.Row>
									<Table.Cell class="h-10 align-middle font-medium">{tr.dict["Model Size"] || "Model Size"}</Table.Cell>
							...
							{#if modelMeta?.n_params}
								<Table.Row>
									<Table.Cell class="h-10 align-middle font-medium">{tr.dict["Parameters"] || "Parameters"}</Table.Cell>
							...
							{#if modelMeta?.n_embd}
								<Table.Row>
									<Table.Cell class="align-middle font-medium">{tr.dict["Embedding Size"] || "Embedding Size"}</Table.Cell>
							...
							{#if modelMeta?.n_vocab}
								<Table.Row>
									<Table.Cell class="align-middle font-medium">{tr.dict["Vocabulary Size"] || "Vocabulary Size"}</Table.Cell>
							...
							{#if modelMeta?.vocab_type}
								<Table.Row>
									<Table.Cell class="align-middle font-medium">{tr.dict["Vocabulary Type"] || "Vocabulary Type"}</Table.Cell>
							...
							<!-- Total Slots -->
							<Table.Row>
								<Table.Cell class="align-middle font-medium">{tr.dict["Parallel Slots"] || "Parallel Slots"}</Table.Cell>
							...
							{#if modalities.length > 0}
								<Table.Row>
									<Table.Cell class="align-middle font-medium">{tr.dict["Modalities"] || "Modalities"}</Table.Cell>
							...
							<!-- Build Info -->
							<Table.Row>
								<Table.Cell class="align-middle font-medium">{tr.dict["Build Info"] || "Build Info"}</Table.Cell>
							...
							{#if serverProps.chat_template}
								<Table.Row>
									<Table.Cell class="align-middle font-medium">{tr.dict["Chat Template"] || "Chat Template"}</Table.Cell>
			...
			{:else if !isLoadingModels}
				<div class="flex items-center justify-center py-8">
					<div class="text-sm text-muted-foreground">{tr.dict["No model information available"] || "No model information available"}</div>
				</div>
			{/if}
		</div>
	</Dialog.Content>

	...
	<AlertDialog.Content class="max-w-lg">
		<AlertDialog.Header>
			<AlertDialog.Title class="flex items-center gap-2">
				...
				{tr.dict["Model Not Available"] || "Model Not Available"}
			</AlertDialog.Title>

			<AlertDialog.Description>
				{tr.dict["The requested model could not be found. Select an available model to continue."] || "The requested model could not be found. Select an available model to continue."}
			</AlertDialog.Description>
		</AlertDialog.Header>
		...
		<div class="space-y-3">
			<div class="rounded-lg border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-sm">
				<p class="font-medium text-amber-600 dark:text-amber-400">
					{tr.dict["Requested:"] || "Requested:"} <code class="rounded bg-amber-500/20 px-1.5 py-0.5">{modelName}</code>
				</p>
			</div>

			{#if availableModels.length > 0}
				<div class="text-sm">
					<p class="mb-2 font-medium text-muted-foreground">{tr.dict["Select an available model:"] || "Select an available model:"}</p>
		...
		<AlertDialog.Footer>
			<AlertDialog.Action onclick={() => handleOpenChange(false)}>{tr.dict["Cancel"] || "Cancel"}</AlertDialog.Action>
		</AlertDialog.Footer>
	</AlertDialog.Content>
</AlertDialog.Root>

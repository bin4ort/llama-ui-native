...
<div class={['flex flex-col gap-3', className]}>
	{#if !resource}
		<div class="flex flex-col items-center justify-center gap-2 py-8 text-muted-foreground">
			...
			<span class="text-sm">{tr.dict["Select a resource to preview"] || "Select a resource to preview"}</span>
		</div>
	{:else}
		<div class="flex items-start justify-between gap-2">
			...
			<div class="flex items-center gap-1">
				<ActionIconCopyToClipboard
					...
					ariaLabel={tr.dict["Copy content"] || "Copy content"}
				/>
				...
				<Button
					...
					title={tr.dict["Download content"] || "Download content"}
				>
				...
				{#each blobContent as blob (blob.uri)}
					{#if isImageMimeType(blob.mimeType ?? MimeTypeApplication.OCTET_STREAM)}
						<img
							...
							alt={tr.dict["Resource content"] || "Resource content"}
					...
					{:else}
						<div class="flex items-center gap-2 rounded bg-muted p-2 text-sm text-muted-foreground">
							...
							<span>{tr.dict["Binary content"] || "Binary content"} ({blob.mimeType || t("unknown type")})</span>
						</div>
					{/if}
				{/each}

				{#if !textContent && blobContent.length === 0}
					<div class="py-4 text-center text-sm text-muted-foreground">{tr.dict["No content available"] || "No content available"}</div>
				{/if}
			{/if}
		</div>
			...
			<div class="flex flex-wrap gap-2 text-xs text-muted-foreground">
				...
				{#if resource.annotations?.priority !== undefined}
					<span class="rounded bg-muted px-1.5 py-0.5">
						{tr.dict["Priority"] || "Priority"}: {resource.annotations.priority}
					</span>
				{/if}

				<span class="rounded bg-muted px-1.5 py-0.5">
					{tr.dict["Server"] || "Server"}: {resource.serverName}
				</span>
			</div>
		{/if}
	{/if}

...
<form onsubmit={handleSubmit} class="space-y-3">
	...
	{#if isComplete}
		<div class="rounded-md bg-muted/50 px-3 py-2">
			<p class="text-xs text-muted-foreground">{tr.dict["Resolved URI"] || "Resolved URI"}:</p>
	...
	<div class="flex justify-end gap-2 pt-1">
		<Button type="button" size="sm" variant="secondary" onclick={onCancel}>{tr.dict["Cancel"] || "Cancel"}</Button>

		<Button size="sm" type="submit" disabled={!isComplete}>{tr.dict["Read Resource"] || "Read Resource"}</Button>
	</div>
</form>

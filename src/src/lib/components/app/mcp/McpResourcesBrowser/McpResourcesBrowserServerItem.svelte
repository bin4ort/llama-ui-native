	...
	>
		...
		<span class="inline-flex flex-col items-start gap-1 text-left">
			...
			<span class="text-xs text-muted-foreground">
				({serverRes.resources.length} {tr.dict["resource"] || "resource"}{serverRes.resources.length !== 1
					? t("s")
					: ''}{#if hasTemplates}, {serverRes.templates.length} {tr.dict["template"] || "template"}{serverRes.templates
						.length !== 1
						? t("s")
						: ''}{/if})
			</span>
		...
		<div class="ml-4 flex flex-col gap-0.5 border-l border-border/50 pl-2">
			{#if serverRes.error}
				<div class="py-1 text-xs text-red-500">
					{tr.dict["Error"] || "Error"}: {serverRes.error}
				</div>
			{:else if !hasContent}
				<div class="py-1 text-xs text-muted-foreground">{tr.dict["No resources"] || "No resources"}</div>
			{:else}
				{#if hasResources}

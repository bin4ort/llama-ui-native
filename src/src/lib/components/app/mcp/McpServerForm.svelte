<script lang="ts">
	...
	let {
		...
		name = '',
		onNameChange,
		namePlaceholder = t("Name reported by the server"),
		useProxy = false,
		onUrlChange,
		onHeadersChange,
		onUseProxyChange,
...
<div class="grid gap-2">
	<div class="mb-4">
		<label for="server-url-{id}" class="mb-2 block text-xs font-medium select-none">
				{tr.dict["Server URL"] || "Server URL"} <span class="text-destructive">*</span>
			</label>
	...
	<div class="mb-4">
		<label for="server-name-{id}" class="mb-2 block text-xs font-medium select-none">
				{tr.dict["Display name"] || "Display name"}
			</label>
	...
	<label class="flex items-center gap-2 cursor-pointer select-none">
		...
		<span class="text-xs text-muted-foreground">
				{tr.dict["Authorization"] || "Authorization"}{#if required}
	...
	{#if showAuthorization}
		<div class="relative mt-2">
			<Input
				...
				type="password"
				autocomplete="off"
				placeholder={tr.dict["Paste token here"] || "Paste token here"}
				value={bearerToken}
	...
	<KeyValuePairs
		...
		keyPlaceholder={tr.dict["Header name"] || "Header name"}
		valuePlaceholder={tr.dict["Value"] || "Value"}
		addButtonLabel={tr.dict["Add"] || "Add"}
		emptyMessage={tr.dict["No custom headers configured."] || "No custom headers configured."}
		sectionLabel={tr.dict["Custom Headers"] || "Custom Headers"}
	...
	{#if !isWebSocket && onUseProxyChange}
		...
		>
			...
			<span>
				<span class="text-xs text-muted-foreground">{tr.dict["Use llama-server proxy"] || "Use llama-server proxy"}</span>

				<br />

				{#if !mcpStore.isProxyAvailable}
					<span class="inline-flex gap-0.75 text-xs text-muted-foreground/60"
						>({tr.dict["Run"] || "Run"} <pre>llama-server</pre>
						{tr.dict["with"] || "with"}
						<pre>{CLI_FLAGS.MCP_PROXY}</pre>
						{tr.dict["flag"] || "flag"})</span
					>
				{/if}
			</span>
		</label>

	...
	>
		...
		{#if depth > 0}
			<Tooltip.Root>
				...
				<Tooltip.Content>
					<p>{tr.dict["See parent conversation"] || "See parent conversation"}</p>
				</Tooltip.Content>
			</Tooltip.Root>
		{/if}
			...
			<Tooltip.Root>
				<Tooltip.Trigger>
					<div
						...
						role="button"
						tabindex="0"
						aria-label={tr.dict["Stop generation"] || "Stop generation"}
					>
				...
				<Tooltip.Content>
					<p>{tr.dict["Stop generation"] || "Stop generation"}</p>
				</Tooltip.Content>
			</Tooltip.Root>
		{/if}
	...
	{#if !isSelectionMode && renderActionsDropdown}
		<div class="actions flex items-center">
			<DropdownMenuActions
				...
				triggerTooltip={tr.dict["More actions"] || "More actions"}

...
<div
	role="toolbar"
	aria-label={tr.dict["Bulk actions for selected conversations"] || "Bulk actions for selected conversations"}
...
>
	<label class="flex min-w-0 cursor-pointer items-center gap-2">
		...
		<span class="truncate text-xs font-medium text-muted-foreground">
			{selectedCount} / {visibleCount} {tr.dict["selected"] || "selected"}
		</span>
	</label>
	...
	<div class="ml-auto flex items-center gap-0.75">
		...
		<ActionIcon
			...
			ariaLabel={tr.dict["Export selected"] || "Export selected"}
			size="sm"
			iconSize="h-3.5 w-3.5"
		...
		<ActionIcon
			icon={Trash2}
			tooltip={tr.dict["Delete selected"] || "Delete selected"}
			tooltipSide={TooltipSide.TOP}
			disabled={!hasSelection}
			ariaLabel={tr.dict["Delete selected"] || "Delete selected"}
			size="sm"
		...
		<ActionIcon
			icon={X}
			tooltip={tr.dict["Exit bulk selection mode"] || "Exit bulk selection mode"}
			tooltipSide={TooltipSide.TOP}
			ariaLabel={tr.dict["Exit bulk selection mode"] || "Exit bulk selection mode"}
			size="sm"
			iconSize="h-3.5 w-3.5"
